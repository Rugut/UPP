...
namespace V82.СправочникиСсылка
{
	...
	public partial class ОснованияУвольненияИзОрганизации:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public ОснованияУвольненияИзОрганизации()
		{
		}
		
		public ОснованияУвольненияИзОрганизации(byte[] УникальныйИдентификатор)
			: this(УникальныйИдентификатор,0)
		{
		}
		
		public ОснованияУвольненияИзОрганизации(byte[] УникальныйИдентификатор,int Глубина)
		{
			if (Глубина>3)
			{
				return;
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							ТекстОснования = Читалка.GetString(5);
							//return Ссылка;
						}
						else
						{
		...
		public static V82.СправочникиСсылка.ОснованияУвольненияИзОрганизации ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ОснованияУвольненияИзОрганизации)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ОснованияУвольненияИзОрганизации(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
