...
namespace V82.СправочникиСсылка
{
	...
	public partial class ДокументыУдостоверяющиеЛичность:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public ДокументыУдостоверяющиеЛичность()
		{
		}
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Наименование = Читалка.GetString(4);
								КодИМНС = Читалка.GetString(5);
								КодПФР = Читалка.GetString(6);
							//return Ссылка;
						}
						else
						{
		...
		public static V82.СправочникиСсылка.ДокументыУдостоверяющиеЛичность ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ДокументыУдостоверяющиеЛичность)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ДокументыУдостоверяющиеЛичность(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
