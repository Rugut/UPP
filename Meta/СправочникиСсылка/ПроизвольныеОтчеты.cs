...
namespace V82.СправочникиСсылка
{
	...
	public partial class ПроизвольныеОтчеты:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public V82.СправочникиСсылка.ПроизвольныеОтчеты Родитель {get;set;}
		...
		public V82.Перечисления/*Ссылка*/.ВидыПроизвольныхОтчетов ВидПроизвольногоОтчета {get;set;}//Вид произвольного отчета
		
		public ПроизвольныеОтчеты()
		{
		}
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.ПроизвольныеОтчеты((byte[])Читалка.GetValue(4),Глубина+1);
							ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Наименование = Читалка.GetString(6);
							if(!ЭтоГруппа)
							{
								Описание = Читалка.GetString(8);
								ВидПроизвольногоОтчета = V82.Перечисления/*Ссылка*/.ВидыПроизвольныхОтчетов.ПустаяСсылка.Получить((byte[])Читалка.GetValue(9));
							}
							//return Ссылка;
						}
						else
		...
		public V82.СправочникиОбъект.ПроизвольныеОтчеты  ПолучитьОбъект()
		{
			...
			Объект.ВидПроизвольногоОтчета = ВидПроизвольногоОтчета;
			return Объект;
		}
		
		private static readonly Hashtable Кэш = new Hashtable(1000);
		
		public static V82.СправочникиСсылка.ПроизвольныеОтчеты ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ПроизвольныеОтчеты)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ПроизвольныеОтчеты(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
