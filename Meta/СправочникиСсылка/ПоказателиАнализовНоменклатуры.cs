...
namespace V82.СправочникиСсылка
{
	...
	public partial class ПоказателиАнализовНоменклатуры:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public ПоказателиАнализовНоменклатуры()
		{
		}
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.ПоказателиАнализовНоменклатуры((byte[])Читалка.GetValue(4),Глубина+1);
							ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Код = Читалка.GetString(6);
							Наименование = Читалка.GetString(7);
							if(!ЭтоГруппа)
							{
								МинЗначение = Читалка.GetDecimal(9);
								МаксЗначение = Читалка.GetDecimal(10);
								ЕдиницаИзмерения = new V82.СправочникиСсылка.КлассификаторЕдиницИзмерения((byte[])Читалка.GetValue(11),Глубина+1);
							}
							//return Ссылка;
						}
						else
		...
		public static V82.СправочникиСсылка.ПоказателиАнализовНоменклатуры ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ПоказателиАнализовНоменклатуры)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ПоказателиАнализовНоменклатуры(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
