...
namespace V82.СправочникиСсылка
{
	...
	public partial class ЦеновыеГруппы:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public V82.СправочникиСсылка.ЦеновыеГруппы Родитель {get;set;}
		...
		public ЦеновыеГруппы()
		{
		}
		
		public ЦеновыеГруппы(byte[] УникальныйИдентификатор)
			: this(УникальныйИдентификатор,0)
		{
		}
		
		public ЦеновыеГруппы(byte[] УникальныйИдентификатор,int Глубина)
		{
			if (Глубина>3)
			{
				return;
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.ЦеновыеГруппы((byte[])Читалка.GetValue(4),Глубина+1);
							ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Код = Читалка.GetString(6);
							Наименование = Читалка.GetString(7);
								Порядок = Читалка.GetDecimal(8);
							//return Ссылка;
						}
						else
						{
		...
		public static V82.СправочникиСсылка.ЦеновыеГруппы ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ЦеновыеГруппы)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ЦеновыеГруппы(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
