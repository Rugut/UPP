				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							ВидПрочихДоходовИРасходов = V82.Перечисления/*Ссылка*/.ВидыПрочихДоходовИРасходов.ПустаяСсылка.Получить((byte[])Читалка.GetValue(6));
							ПринятиеКналоговомуУчету = ((byte[])Читалка.GetValue(7))[0]==1;
							//return Ссылка;
						}
						else
						{
