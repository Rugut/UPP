...
namespace V82.СправочникиСсылка
{
	...
	public partial class ПрочиеДоходыИРасходы:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public V82.СправочникиСсылка.ПрочиеДоходыИРасходы Родитель {get;set;}
		...
		public ПрочиеДоходыИРасходы()
		{
		}
		
		public ПрочиеДоходыИРасходы(byte[] УникальныйИдентификатор)
			: this(УникальныйИдентификатор,0)
		{
		}
		
		public ПрочиеДоходыИРасходы(byte[] УникальныйИдентификатор,int Глубина)
		{
			if (Глубина>3)
			{
				return;
			...
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_Fld1684RRef [ВидПрочихДоходовИРасходов]
					,_Fld1685 [ПринятиеКналоговомуУчету]
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.ПрочиеДоходыИРасходы((byte[])Читалка.GetValue(4),Глубина+1);
							ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Код = Читалка.GetString(6);
							Наименование = Читалка.GetString(7);
							if(!ЭтоГруппа)
							{
								ВидПрочихДоходовИРасходов = V82.Перечисления/*Ссылка*/.ВидыПрочихДоходовИРасходов.ПустаяСсылка.Получить((byte[])Читалка.GetValue(8));
								ПринятиеКналоговомуУчету = ((byte[])Читалка.GetValue(9))[0]==1;
							}
							//return Ссылка;
						}
						else
		...
		public static V82.СправочникиСсылка.ПрочиеДоходыИРасходы ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ПрочиеДоходыИРасходы)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ПрочиеДоходыИРасходы(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
