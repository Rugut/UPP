...
namespace V82.СправочникиСсылка
{
	...
	public partial class ПрофилиПолномочийПользователей:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public ПрофилиПолномочийПользователей()
		{
		}
		
		public ПрофилиПолномочийПользователей(byte[] УникальныйИдентификатор)
			: this(УникальныйИдентификатор,0)
		{
		}
		
		public ПрофилиПолномочийПользователей(byte[] УникальныйИдентификатор,int Глубина)
		{
			if (Глубина>3)
			{
				return;
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.ПрофилиПолномочийПользователей((byte[])Читалка.GetValue(4),Глубина+1);
							ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Наименование = Читалка.GetString(6);
							if(!ЭтоГруппа)
							{
								ОсновнойИнтерфейс = Читалка.GetString(7);
							}
							//return Ссылка;
						}
						else
		...
		public static V82.СправочникиСсылка.ПрофилиПолномочийПользователей ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ПрофилиПолномочийПользователей)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ПрофилиПолномочийПользователей(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
