...
namespace V82.СправочникиСсылка
{
	...
	public partial class ТарифыБанковНаДенежныеПереводы:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public ТарифыБанковНаДенежныеПереводы()
		{
		}
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Наименование = Читалка.GetString(5);
								МинимальныйТариф = Читалка.GetDecimal(6);
								МаксимальныйТариф = Читалка.GetDecimal(7);
								Процент = Читалка.GetDecimal(8);
							//return Ссылка;
						}
						else
						{
		...
		public static V82.СправочникиСсылка.ТарифыБанковНаДенежныеПереводы ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ТарифыБанковНаДенежныеПереводы)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ТарифыБанковНаДенежныеПереводы(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
