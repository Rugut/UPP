...
namespace V82.СправочникиСсылка
{
	...
	public partial class СтатьиОтчетаОПрибыляхИУбытках:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public СтатьиОтчетаОПрибыляхИУбытках()
		{
		}
		
		public СтатьиОтчетаОПрибыляхИУбытках(byte[] УникальныйИдентификатор)
			: this(УникальныйИдентификатор,0)
		{
		}
		
		public СтатьиОтчетаОПрибыляхИУбытках(byte[] УникальныйИдентификатор,int Глубина)
		{
			if (Глубина>3)
			{
				return;
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Наименование = Читалка.GetString(5);
								ОбластьОтчета = Читалка.GetString(6);
								НаименованиеДляОтчета = Читалка.GetString(7);
							//return Ссылка;
						}
						else
						{
		...
		public static V82.СправочникиСсылка.СтатьиОтчетаОПрибыляхИУбытках ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.СтатьиОтчетаОПрибыляхИУбытках)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.СтатьиОтчетаОПрибыляхИУбытках(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
