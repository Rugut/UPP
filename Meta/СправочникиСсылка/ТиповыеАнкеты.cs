				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.ТиповыеАнкеты((byte[])Читалка.GetValue(4),Глубина+1);
							ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Код = Читалка.GetString(6);
							Наименование = Читалка.GetString(7);
							if(!ЭтоГруппа)
							{
								ЗагружатьОбъекты = ((byte[])Читалка.GetValue(8))[0]==1;
								ВидСправочникаДляЗагрузки = V82.Перечисления/*Ссылка*/.ВидыОбъектовЗагружаемыхИзОпроса.ПустаяСсылка.Получить((byte[])Читалка.GetValue(10));
								Адресная = ((byte[])Читалка.GetValue(11))[0]==1;
								НаименованиеАнкеты = Читалка.GetString(12);
								Вступление = Читалка.GetString(13);
								ЗаполнятьАнкетуДанными = ((byte[])Читалка.GetValue(14))[0]==1;
								ВидАнкеты = V82.Перечисления/*Ссылка*/.ВидыАнкет.ПустаяСсылка.Получить((byte[])Читалка.GetValue(15));
								СекундОтвета = Читалка.GetDecimal(16);
							}
							//return Ссылка;
						}
						else
