...
namespace V82.СправочникиСсылка
{
	...
	public partial class ПодразделенияОрганизаций:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public V82.СправочникиСсылка.ПодразделенияОрганизаций Родитель {get;set;}
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.ПодразделенияОрганизаций((byte[])Читалка.GetValue(4),Глубина+1);
							Код = Читалка.GetString(5);
							Наименование = Читалка.GetString(6);
								ВидПодразделения = V82.Перечисления/*Ссылка*/.ВидыПодразделений.ПустаяСсылка.Получить((byte[])Читалка.GetValue(7));
								КодПоОКАТО = Читалка.GetString(8);
								КПП = Читалка.GetString(9);
								РайонныйКоэффициент = Читалка.GetDecimal(10);
								РайонныйКоэффициентРФ = Читалка.GetDecimal(11);
								ТерриториальныеУсловияПФР = new V82.СправочникиСсылка.ТерриториальныеУсловия((byte[])Читалка.GetValue(12),Глубина+1);
								Порядок = Читалка.GetDecimal(13);
								СоответствуетСудамПодФлагомРФ = ((byte[])Читалка.GetValue(14))[0]==1;
							//return Ссылка;
						}
						else
						{
