...
namespace V82.СправочникиСсылка
{
	...
	public partial class АдресныеСокращения:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Уровень = Читалка.GetDecimal(6);
							Сокращение = Читалка.GetString(7);
							//return Ссылка;
						}
						else
						{
		...
		public static V82.СправочникиСсылка.АдресныеСокращения ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.АдресныеСокращения)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.АдресныеСокращения(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
