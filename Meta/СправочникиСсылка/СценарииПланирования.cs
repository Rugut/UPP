				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.СценарииПланирования((byte[])Читалка.GetValue(4),Глубина+1);
							...
							if(!ЭтоГруппа)
							{
								...
								ДетализацияПланирования = V82.Перечисления/*Ссылка*/.ДетализацияПланирования.ПустаяСсылка.Получить((byte[])Читалка.GetValue(9));
								УчетПоСуммам = ((byte[])Читалка.GetValue(10))[0]==1;
								УчетПоКоличеству = ((byte[])Читалка.GetValue(11))[0]==1;
								Валюта = new V82.СправочникиСсылка.Валюты((byte[])Читалка.GetValue(12),Глубина+1);
								СпособПланирования = V82.Перечисления/*Ссылка*/.СпособыПланирования.ПустаяСсылка.Получить((byte[])Читалка.GetValue(13));
								ПродолжительностьЦикла = Читалка.GetDecimal(14);
								НачалоПлана = Читалка.GetDateTime(15);
								КонецПлана = Читалка.GetDateTime(16);
								НормаДоходности = Читалка.GetDecimal(17);
								ИспользоватьКурсыСценария = ((byte[])Читалка.GetValue(18))[0]==1;
							}
							//return Ссылка;
						}
						else
