...
namespace V82.СправочникиСсылка
{
	...
	public partial class НематериальныеАктивы:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public V82.СправочникиСсылка.НематериальныеАктивы Родитель {get;set;}
		...
		public НематериальныеАктивы(byte[] УникальныйИдентификатор,int Глубина)
		{
			if (Глубина>3)
			{
				return;
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.НематериальныеАктивы((byte[])Читалка.GetValue(4),Глубина+1);
							...
							if(!ЭтоГруппа)
							{
								...
								ВидНМА = V82.Перечисления/*Ссылка*/.ВидыНМА.ПустаяСсылка.Получить((byte[])Читалка.GetValue(9));
								АмортизационнаяГруппа = V82.Перечисления/*Ссылка*/.АмортизационныеГруппы.ПустаяСсылка.Получить((byte[])Читалка.GetValue(10));
								ПрочиеСведения = Читалка.GetString(11);
								ВидОбъектаУчета = V82.Перечисления/*Ссылка*/.ВидыОбъектовУчетаНМА.ПустаяСсылка.Получить((byte[])Читалка.GetValue(12));
							}
							//return Ссылка;
						}
						else
		...
		public static V82.СправочникиСсылка.НематериальныеАктивы ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.НематериальныеАктивы)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.НематериальныеАктивы(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
