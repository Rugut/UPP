...
namespace V82.СправочникиСсылка
{
	...
	public partial class ИнформационныеКарты:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public V82.СправочникиСсылка.ИнформационныеКарты Родитель {get;set;}
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.ИнформационныеКарты((byte[])Читалка.GetValue(4),Глубина+1);
							ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Код = Читалка.GetString(6);
							Наименование = Читалка.GetString(7);
							if(!ЭтоГруппа)
							{
								КодКарты = Читалка.GetString(8);
								ВидКарты = V82.Перечисления/*Ссылка*/.ВидыИнформационныхКарт.ПустаяСсылка.Получить((byte[])Читалка.GetValue(12));
								ТипКарты = V82.Перечисления/*Ссылка*/.ТипыИнформационныхКарт.ПустаяСсылка.Получить((byte[])Читалка.GetValue(13));
								ВидДисконтнойКарты = new V82.СправочникиСсылка.ВидыДисконтныхКарт((byte[])Читалка.GetValue(15),Глубина+1);
							}
							//return Ссылка;
						}
						else
