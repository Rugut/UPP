...
namespace V82.СправочникиСсылка
{
	...
	public partial class ФильтрыДляЭлектронныхПисем:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public ФильтрыДляЭлектронныхПисем()
		{
		}
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							ОперацияУсловия = ((byte[])Читалка.GetValue(6))[0]==1;
							Порядок = Читалка.GetDecimal(7);
							Использование = ((byte[])Читалка.GetValue(8))[0]==1;
							//return Ссылка;
						}
						else
						{
		...
		public static V82.СправочникиСсылка.ФильтрыДляЭлектронныхПисем ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ФильтрыДляЭлектронныхПисем)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ФильтрыДляЭлектронныхПисем(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
