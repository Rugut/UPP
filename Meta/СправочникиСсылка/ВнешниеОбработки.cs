...
namespace V82.СправочникиСсылка
{
	...
	public partial class ВнешниеОбработки:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public V82.СправочникиСсылка.ВнешниеОбработки Родитель {get;set;}
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.ВнешниеОбработки((byte[])Читалка.GetValue(4),Глубина+1);
							ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Код = Читалка.GetString(6);
							Наименование = Читалка.GetString(7);
							if(!ЭтоГруппа)
							{
								ВидОбработки = V82.Перечисления/*Ссылка*/.ВидыДополнительныхВнешнихОбработок.ПустаяСсылка.Получить((byte[])Читалка.GetValue(9));
								Комментарий = Читалка.GetString(10);
								КомментарийКФайлуИсточнику = Читалка.GetString(11);
							}
							//return Ссылка;
						}
						else
		...
		public V82.СправочникиОбъект.ВнешниеОбработки  ПолучитьОбъект()
		{
			...
			Объект.ВидОбработки = ВидОбработки;
			Объект.Комментарий = Комментарий;
			Объект.КомментарийКФайлуИсточнику = КомментарийКФайлуИсточнику;
			return Объект;
		}
		
		private static readonly Hashtable Кэш = new Hashtable(1000);
		
		public static V82.СправочникиСсылка.ВнешниеОбработки ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ВнешниеОбработки)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ВнешниеОбработки(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
