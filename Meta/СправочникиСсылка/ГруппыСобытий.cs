...
namespace V82.СправочникиСсылка
{
	...
	public partial class ГруппыСобытий:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public ГруппыСобытий()
		{
		}
		
		public ГруппыСобытий(byte[] УникальныйИдентификатор)
			: this(УникальныйИдентификатор,0)
		{
		}
		
		public ГруппыСобытий(byte[] УникальныйИдентификатор,int Глубина)
		{
			if (Глубина>3)
			{
				return;
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							ОписаниеСобытия = Читалка.GetString(6);
							ВидОбъекта = V82.Перечисления/*Ссылка*/.ВидыОбъектовСобытия.ПустаяСсылка.Получить((byte[])Читалка.GetValue(7));
							//return Ссылка;
						}
						else
						{
		...
		public static V82.СправочникиСсылка.ГруппыСобытий ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ГруппыСобытий)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ГруппыСобытий(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
