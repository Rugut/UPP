...
namespace V82.СправочникиСсылка
{
	...
	public partial class ГруппыПисемЭлектроннойПочты:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public ГруппыПисемЭлектроннойПочты()
		{
		}
		
		public ГруппыПисемЭлектроннойПочты(byte[] УникальныйИдентификатор)
			: this(УникальныйИдентификатор,0)
		{
		}
		
		public ГруппыПисемЭлектроннойПочты(byte[] УникальныйИдентификатор,int Глубина)
		{
			if (Глубина>3)
			{
				return;
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.ГруппыПисемЭлектроннойПочты((byte[])Читалка.GetValue(4),Глубина+1);
							Код = Читалка.GetString(5);
							Наименование = Читалка.GetString(6);
								Порядок = Читалка.GetDecimal(7);
								ИспользоватьПредметыПисем = ((byte[])Читалка.GetValue(8))[0]==1;
							//return Ссылка;
						}
						else
						{
