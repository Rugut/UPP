...
namespace V82.СправочникиСсылка
{
	...
	public partial class Кассы:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public V82.СправочникиСсылка.Кассы Родитель {get;set;}
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.Кассы((byte[])Читалка.GetValue(4),Глубина+1);
							ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Код = Читалка.GetString(6);
							Наименование = Читалка.GetString(7);
							if(!ЭтоГруппа)
							{
								ВалютаДенежныхСредств = new V82.СправочникиСсылка.Валюты((byte[])Читалка.GetValue(8),Глубина+1);
							}
							//return Ссылка;
						}
						else
		...
		public static V82.СправочникиСсылка.Кассы ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.Кассы)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.Кассы(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
