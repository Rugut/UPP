...
namespace V82.СправочникиСсылка
{
	...
	public partial class ВидыЦФО:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public V82.СправочникиСсылка.ВидыЦФО Родитель {get;set;}
		...
		public ВидыЦФО(byte[] УникальныйИдентификатор)
			: this(УникальныйИдентификатор,0)
		{
		}
		
		public ВидыЦФО(byte[] УникальныйИдентификатор,int Глубина)
		{
			if (Глубина>3)
			{
				return;
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.ВидыЦФО((byte[])Читалка.GetValue(4),Глубина+1);
		...
		public static V82.СправочникиСсылка.ВидыЦФО ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.ВидыЦФО)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.ВидыЦФО(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
