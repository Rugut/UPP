				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.КлючевыеРесурсыПредприятия((byte[])Читалка.GetValue(4),Глубина+1);
							ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Код = Читалка.GetString(6);
							Наименование = Читалка.GetString(7);
							if(!ЭтоГруппа)
							{
								БазоваяЕдиницаИзмерения = new V82.СправочникиСсылка.КлассификаторЕдиницИзмерения((byte[])Читалка.GetValue(8),Глубина+1);
								БазаЗаданияПотребности = V82.Перечисления/*Ссылка*/.БазыЗаданияПотребностейПоКлючевымРесурсам.ПустаяСсылка.Получить((byte[])Читалка.GetValue(9));
							}
							//return Ссылка;
						}
						else
