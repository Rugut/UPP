...
namespace V82.СправочникиСсылка
{
	...
	public partial class КомпетенцииРаботников:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Родитель = new V82.СправочникиСсылка.КомпетенцииРаботников((byte[])Читалка.GetValue(4),Глубина+1);
							ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Наименование = Читалка.GetString(6);
							if(!ЭтоГруппа)
							{
								ОписаниеКомпетенции = Читалка.GetString(7);
								ШкалаОценок = new V82.СправочникиСсылка.ШкалыОценокКомпетенций((byte[])Читалка.GetValue(8),Глубина+1);
								ТиповаяАнкета = new V82.СправочникиСсылка.ТиповыеАнкеты((byte[])Читалка.GetValue(9),Глубина+1);
							}
							//return Ссылка;
						}
						else
		...
		public static V82.СправочникиСсылка.КомпетенцииРаботников ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.КомпетенцииРаботников)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.КомпетенцииРаботников(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
