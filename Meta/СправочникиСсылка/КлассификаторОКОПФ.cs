...
namespace V82.СправочникиСсылка
{
	...
	public partial class КлассификаторОКОПФ:СправочникСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							БыстрыйВыбор = ((byte[])Читалка.GetValue(7))[0]==1;
							//return Ссылка;
						}
						else
						{
		...
		public static V82.СправочникиСсылка.КлассификаторОКОПФ ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.СправочникиСсылка.КлассификаторОКОПФ)Кэш[УИ];
			}
			var Ссылка = new V82.СправочникиСсылка.КлассификаторОКОПФ(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
