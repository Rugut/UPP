...
namespace V82.Справочники//Менеджер
{
	...
	public partial class ДоходыНДФЛ:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.ДоходыНДФЛ НайтиПоСсылке(Guid _Ссылка)
		{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.Наименование = Читалка.GetString(5);
								...
								Ссылка.НеОблагаетсяУНалоговогоАгента = ((byte[])Читалка.GetValue(8))[0]==1;
								Ссылка.НеОтражаетсяВОтчетности2010 = ((byte[])Читалка.GetValue(9))[0]==1;
								Ссылка.УчитыватьПоПериодуДействия = ((byte[])Читалка.GetValue(10))[0]==1;
							return Ссылка;
						}
						else
						{
		...
		public static СправочникиВыборка.ДоходыНДФЛ СтраницаПоСсылке(int Размер,int Номер)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
		...
		public static СправочникиВыборка.ДоходыНДФЛ СтраницаПоКоду(int Размер,int Номер)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
		...
		public static СправочникиВыборка.ДоходыНДФЛ СтраницаПоНаименованию(int Размер,int Номер)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
		...
		public static СправочникиВыборка.ДоходыНДФЛ ИерархияВыбратьПоСсылке(Guid Родитель,int Режим,int Первые,Guid Мин,Guid Макс)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
		...
		public static СправочникиВыборка.ДоходыНДФЛ ИерархияСтраницаПоСсылке(Guid Родитель,int Режим,int Размер,int Номер)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
