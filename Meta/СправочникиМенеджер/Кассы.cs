...
namespace V82.Справочники//Менеджер
{
	...
	public partial class Кассы:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.Кассы НайтиПоСсылке(Guid _Ссылка)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.Родитель = V82.СправочникиСсылка.Кассы.ВзятьИзКэша((byte[])Читалка.GetValue(4));
							Ссылка.ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Ссылка.Код = Читалка.GetString(6);
							Ссылка.Наименование = Читалка.GetString(7);
							if(!Ссылка.ЭтоГруппа)
							{
								Ссылка.ВалютаДенежныхСредств = V82.СправочникиСсылка.Валюты.ВзятьИзКэша((byte[])Читалка.GetValue(8));
							}
							return Ссылка;
						}
						else
		...
		public static СправочникиВыборка.Кассы Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_Description [Наименование]
					,_Fld1195RRef [ВалютаДенежныхСредств]
					From _Reference65(NOLOCK)  Where _Folder = 0x01 ";
		...
		public static СправочникиВыборка.Кассы СтраницаПоСсылке(int Размер,int Номер)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
		...
		public static СправочникиВыборка.Кассы СтраницаПоКоду(int Размер,int Номер)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
		...
		public static СправочникиВыборка.Кассы СтраницаПоНаименованию(int Размер,int Номер)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
		...
		public static V82.СправочникиОбъект.Кассы СоздатьГруппу()
		{
			var Объект = new V82.СправочникиОбъект.Кассы();
			Объект._ЭтоНовый = true;
			Объект.Ссылка = Guid.NewGuid();/*http://msdn.microsoft.com/ru-ru/library/aa379322(VS.85).aspx*/
		...
		public static СправочникиВыборка.Кассы ИерархияВыбратьПоСсылке(Guid Родитель,int Режим,int Первые,Guid Мин,Guid Макс)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
		...
		public static СправочникиВыборка.Кассы ИерархияСтраницаПоСсылке(Guid Родитель,int Режим,int Размер,int Номер)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
