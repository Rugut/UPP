		...
		public static СправочникиВыборка.НастройкиФормированияДокументовОтработанногоВремени Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_Description [Наименование],_Fld2871 [ГраницаОбработки],_Fld2872RRef [ПериодДетализации],_Fld2873RRef [Организация],_Fld2874RRef [СпособВводаДанных],_Fld2875RRef [ПодразделениеОрганизации],_Fld2876 [ФормироватьДокументыАвтоматически],_Fld2877 [РегламентноеЗадание],_Fld2878 [НеОбрабатыватьВсеДокументы],_Fld2879 [Комментарий],_Fld2880 [Задержка]
		 From _Reference159(NOLOCK)";
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						while (Читалка.Read())
						{
							...
							//Ссылка.ПодразделениеОрганизации = new V82.СправочникиСсылка.ПодразделенияОрганизаций((byte[])Читалка.GetValue(9));
							Ссылка.ФормироватьДокументыАвтоматически = ((byte[])Читалка.GetValue(10))[0]==1?true:false;
							Ссылка.РегламентноеЗадание = Читалка.GetString(11);
							Ссылка.НеОбрабатыватьВсеДокументы = ((byte[])Читалка.GetValue(12))[0]==1?true:false;
