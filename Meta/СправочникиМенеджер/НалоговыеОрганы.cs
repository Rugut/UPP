...
namespace V82.Справочники//Менеджер
{
	public partial class НалоговыеОрганы:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.НалоговыеОрганы НайтиПоКоду(decimal Код)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.УчетНалогоплательщиков = ((byte[])Читалка.GetValue(8))[0]==1?true:false;
							Ссылка.ПриемНалоговойОтчетности = ((byte[])Читалка.GetValue(9))[0]==1?true:false;
		...
		public static СправочникиВыборка.НалоговыеОрганы Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld2574RRef [Вид],_Fld2575 [ПолноеНаименование],_Fld2576 [УчетНалогоплательщиков],_Fld2577 [ПриемНалоговойОтчетности],_Fld2578 [ИНН],_Fld2579 [КПП],_Fld2580 [Адрес],_Fld2581 [Телефон],_Fld2582 [АдресЭлектроннойПочты],_Fld2583 [АдресСайта],_Fld2584 [Комментарий],_Fld2585 [УдалитьСертификат],_Fld2586 [АдресЭлектроннойПочтыДляЦелейДокументооборотаСНалогоплательщиками]
		 From _Reference144(NOLOCK)";
