...
namespace V82.Справочники//Менеджер
{
	public partial class ВидыОтправляемыхДокументов:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.ВидыОтправляемыхДокументов НайтиПоСсылке(Guid _Ссылка)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.Родитель = V82.СправочникиСсылка.ВидыОтправляемыхДокументов.ВзятьИзКэша((byte[])Читалка.GetValue(4));
							...
							if(!Ссылка.ЭтоГруппа)
							{
								Ссылка.Источник = Читалка.GetString(9);
								Ссылка.ТипПолучателя = V82.Перечисления/*Ссылка*/.ТипыКонтролирующихОрганов.ПустаяСсылка.Получить((byte[])Читалка.GetValue(10));
								Ссылка.ТипДокумента = V82.Перечисления/*Ссылка*/.ТипыОтправляемыхДокументов.ПустаяСсылка.Получить((byte[])Читалка.GetValue(11));
							}
							return Ссылка;
						}
						else
		...
		public static V82.СправочникиОбъект.ВидыОтправляемыхДокументов СоздатьГруппу()
		{
			var Объект = new V82.СправочникиОбъект.ВидыОтправляемыхДокументов();
			Объект._ЭтоНовый = true;
			Объект.Ссылка = Guid.NewGuid();/*http://msdn.microsoft.com/ru-ru/library/aa379322(VS.85).aspx*/
		...
		public static СправочникиВыборка.ВидыОтправляемыхДокументов ИерархияВыбратьПоСсылке(Guid Родитель,int Режим,int Первые,Guid Мин,Guid Макс)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
		...
		public static СправочникиВыборка.ВидыОтправляемыхДокументов ИерархияСтраницаПоСсылке(Guid Родитель,int Режим,int Размер,int Номер)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
