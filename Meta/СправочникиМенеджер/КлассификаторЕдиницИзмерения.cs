...
namespace V82.Справочники//Менеджер
{
	...
	public partial class КлассификаторЕдиницИзмерения:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.КлассификаторЕдиницИзмерения НайтиПоКоду(decimal Код)
		{
		...
		public static СправочникиВыборка.КлассификаторЕдиницИзмерения Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld2397 [НаименованиеПолное],_Fld2398 [МеждународноеСокращение]
		 From _Reference114(NOLOCK)";
