				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						while (Читалка.Read())
						{
							...
							Ссылка.ВидДокументаОбОбразовании = V82.СправочникиСсылка.ДокументыОбОбразовании.ВзятьИзКэша((byte[])Читалка.GetValue(8));
							Ссылка.Валюта = V82.СправочникиСсылка.Валюты.ВзятьИзКэша((byte[])Читалка.GetValue(9));
							Ссылка.ОтражатьВРегУчете = ((byte[])Читалка.GetValue(10))[0]==1;
							Выборка.Add(Ссылка);
						}
							return Выборка;
					}
