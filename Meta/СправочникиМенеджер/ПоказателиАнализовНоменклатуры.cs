...
namespace V82.Справочники//Менеджер
{
	public partial class ПоказателиАнализовНоменклатуры:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.ПоказателиАнализовНоменклатуры НайтиПоКоду(decimal Код)
		{
		...
		public static СправочникиВыборка.ПоказателиАнализовНоменклатуры Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld3214RRef [ВидРезультатаАнализа],_Fld3215 [МинЗначение],_Fld3216 [МаксЗначение],_Fld3217RRef [ЕдиницаИзмерения]
		 From _Reference193(NOLOCK)";
