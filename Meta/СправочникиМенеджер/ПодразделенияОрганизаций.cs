				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.ТерриториальныеУсловияПФР = V82.СправочникиСсылка.ТерриториальныеУсловия.ВзятьИзКэша((byte[])Читалка.GetValue(11));
							Ссылка.Порядок = Читалка.GetDecimal(12);
							Ссылка.СоответствуетСудамПодФлагомРФ = ((byte[])Читалка.GetValue(13))[0]==1;
							return Ссылка;
						}
						else
						{
