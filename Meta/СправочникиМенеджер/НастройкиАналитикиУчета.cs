...
namespace V82.Справочники//Менеджер
{
	public partial class НастройкиАналитикиУчета:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.НастройкиАналитикиУчета НайтиПоКоду(decimal Код)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.ТолькоПросмотр = ((byte[])Читалка.GetValue(9))[0]==1?true:false;
							Ссылка.ТолькоПросмотрРегл = ((byte[])Читалка.GetValue(10))[0]==1?true:false;
							return Ссылка;
						}
						else
						{
		...
		public static СправочникиВыборка.НастройкиАналитикиУчета Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld2641 [УправленческийУчет],_Fld2642 [РегламентированныйУчет],_Fld2643RRef [ЗначениеПоУмолчанию],_Fld2644 [ТолькоПросмотр],_Fld2645 [ТолькоПросмотрРегл]
		 From _Reference148(NOLOCK)";
