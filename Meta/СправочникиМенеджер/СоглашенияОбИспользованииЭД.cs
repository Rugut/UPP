		...
		public static СправочникиВыборка.СоглашенияОбИспользованииЭД Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_Description [Наименование],_Fld3442 [Номер],_Fld3443 [Дата],_Fld3444RRef [СпособОбменаЭД],_Fld3445RRef [СтатусСоглашения],_Fld3446RRef [Организация],_Fld3447RRef [Контрагент],_Fld3448 [Партнер],_Fld3449 [ИдентификаторОрганизации],_Fld3450 [ИдентификаторКонтрагента],_Fld3451RRef [СертификатОрганизацииДляРасшифровки],_Fld3452 [СертификатКонтрагентаДляШифрования],_Fld3453 [АдресРесурсаОрганизации],_Fld3454 [АдресРесурсаКонтрагента],_Fld3455 [СрокДействия],_Fld3456RRef [Ответственный],_Fld3457 [Комментарий],_Fld3458RRef [ТиповоеСоглашение],_Fld3459 [ПроверятьСертификатыПодписей],_Fld3460 [ЭтоТиповое],_Fld3461 [ЭтоИнтеркампани],_Fld3462RRef [СертификатАбонента],_Fld3463 [ЛогинАбонента]
		 From _Reference232(NOLOCK)";
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						while (Читалка.Read())
						{
							...
							Ссылка.АдресРесурсаКонтрагента = Читалка.GetString(17);
							Ссылка.СрокДействия = Читалка.GetDateTime(18);
							...
							//Ссылка.ТиповоеСоглашение = new V82.СправочникиСсылка.СоглашенияОбИспользованииЭД((byte[])Читалка.GetValue(21));
							Ссылка.ПроверятьСертификатыПодписей = ((byte[])Читалка.GetValue(22))[0]==1?true:false;
							Ссылка.ЭтоТиповое = ((byte[])Читалка.GetValue(23))[0]==1?true:false;
							Ссылка.ЭтоИнтеркампани = ((byte[])Читалка.GetValue(24))[0]==1?true:false;
							//Ссылка.СертификатАбонента = new V82.СправочникиСсылка.СертификатыЭЦП((byte[])Читалка.GetValue(25));
							Ссылка.ЛогинАбонента = Читалка.GetString(26);
							Выборка.Add(Ссылка);
						}
							return Выборка;
					}
