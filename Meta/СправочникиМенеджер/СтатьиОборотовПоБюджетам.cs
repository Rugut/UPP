...
namespace V82.Справочники//Менеджер
{
	...
	public partial class СтатьиОборотовПоБюджетам:СправочникМенеджер
	{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.Родитель = V82.СправочникиСсылка.СтатьиОборотовПоБюджетам.ВзятьИзКэша((byte[])Читалка.GetValue(4));
							...
							if(!Ссылка.ЭтоГруппа)
							{
								...
								Ссылка.УчетПоКоличеству = ((byte[])Читалка.GetValue(9))[0]==1;
								Ссылка.УчетПоСумме = ((byte[])Читалка.GetValue(10))[0]==1;
								Ссылка.ИсточникДанныхДляЦены = V82.СправочникиСсылка.ИсточникиДанныхДляРасчетовБюджетирования.ВзятьИзКэша((byte[])Читалка.GetValue(11));
								Ссылка.ПоказательИсточникаДанныхДляЦены = Читалка.GetString(12);
								Ссылка.ИсточникДанныхДляФакта = V82.СправочникиСсылка.ИсточникиДанныхДляРасчетовБюджетирования.ВзятьИзКэша((byte[])Читалка.GetValue(13));
								Ссылка.ПоказательИсточникаДанныхДляФактаСум = Читалка.GetString(14);
								Ссылка.СпособОпределенияВалютыОборотаФакт = V82.Перечисления/*Ссылка*/.СпособыОпределенияВалютыОборотаПоСтатье.ПустаяСсылка.Получить((byte[])Читалка.GetValue(15));
								Ссылка.ПоказательИсточникаДанныхДляФактаКол = Читалка.GetString(16);
								Ссылка.ЕдиницаИзмерения = V82.СправочникиСсылка.КлассификаторЕдиницИзмерения.ВзятьИзКэша((byte[])Читалка.GetValue(17));
								Ссылка.РазделениеПоКонтрагентам = ((byte[])Читалка.GetValue(18))[0]==1;
								Ссылка.РазделениеПоНоменклатуре = ((byte[])Читалка.GetValue(19))[0]==1;
								Ссылка.ОсновнаяВалюта = V82.СправочникиСсылка.Валюты.ВзятьИзКэша((byte[])Читалка.GetValue(20));
								Ссылка.ОсновноеЦФО = V82.СправочникиСсылка.Подразделения.ВзятьИзКэша((byte[])Читалка.GetValue(21));
								Ссылка.ОсновнойПроект = V82.СправочникиСсылка.Проекты.ВзятьИзКэша((byte[])Читалка.GetValue(22));
								Ссылка.ОсновнойКонтрагент = V82.СправочникиСсылка.Контрагенты.ВзятьИзКэша((byte[])Читалка.GetValue(23));
								Ссылка.Комментарий = Читалка.GetString(27);
								Ссылка.КоэффициентДляФактаСум = Читалка.GetDecimal(28);
								Ссылка.КоэффициентДляФактаКол = Читалка.GetDecimal(29);
							}
							return Ссылка;
						}
						else
		...
		public static СправочникиВыборка.СтатьиОборотовПоБюджетам ИерархияВыбратьПоСсылке(Guid Родитель,int Режим,int Первые,Guid Мин,Guid Макс)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
		...
		public static СправочникиВыборка.СтатьиОборотовПоБюджетам ИерархияСтраницаПоСсылке(Guid Родитель,int Режим,int Размер,int Номер)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
