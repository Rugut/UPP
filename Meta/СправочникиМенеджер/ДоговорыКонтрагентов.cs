...
namespace V82.Справочники//Менеджер
{
	...
	public partial class ДоговорыКонтрагентов:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.ДоговорыКонтрагентов НайтиПоКоду(decimal Код)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.ВидДоговора = V82.Перечисления/*Ссылка*/.ВидыДоговоровКонтрагентов.ПустаяСсылка.Получить((byte[])Читалка.GetValue(21));
							Ссылка.ЧислоДнейРезерваБезОплаты = Читалка.GetDecimal(22);
							Ссылка.УчетАгентскогоНДС = ((byte[])Читалка.GetValue(23))[0]==1?true:false;
							Ссылка.ВидАгентскогоДоговора = V82.Перечисления/*Ссылка*/.ВидыАгентскихДоговоров.ПустаяСсылка.Получить((byte[])Читалка.GetValue(24));
							Ссылка.РасчетыВУсловныхЕдиницах = ((byte[])Читалка.GetValue(25))[0]==1?true:false;
							Ссылка.КонтролироватьДенежныеСредстваКомитента = ((byte[])Читалка.GetValue(26))[0]==1?true:false;
							Ссылка.РеализацияНаЭкспорт = ((byte[])Читалка.GetValue(27))[0]==1?true:false;
							Ссылка.Дата = Читалка.GetDateTime(28);
							Ссылка.Номер = Читалка.GetString(29);
							//Ссылка.ВидВзаиморасчетов = new V82.СправочникиСсылка.ВидыВзаиморасчетов((byte[])Читалка.GetValue(30));
							Ссылка.ВестиПоДокументамРасчетовСКонтрагентом = ((byte[])Читалка.GetValue(31))[0]==1?true:false;
							//Ссылка.ОсновнаяСтатьяДвиженияДенежныхСредств = new V82.СправочникиСсылка.СтатьиДвиженияДенежныхСредств((byte[])Читалка.GetValue(33));
							Ссылка.СрокДействия = Читалка.GetDateTime(34);
							//Ссылка.НаименованиеДляСчетаФактурыНаАванс = new V82.СправочникиСсылка.Номенклатура((byte[])Читалка.GetValue(35));
							Ссылка.ПорядокРегистрацииСчетовФактурНаАвансПоДоговору = V82.Перечисления/*Ссылка*/.ПорядокРегистрацииСчетовФактурНаАванс.ПустаяСсылка.Получить((byte[])Читалка.GetValue(36));
							Ссылка.НалоговыйАгентПоОплате = ((byte[])Читалка.GetValue(37))[0]==1?true:false;
							Ссылка.УстановленСрокОплатыДляРезервовПоСомнительнымДолгам = ((byte[])Читалка.GetValue(38))[0]==1?true:false;
							Ссылка.СрокОплатыДляРезервовПоСомнительнымДолгам = Читалка.GetDecimal(39);
							return Ссылка;
						}
						else
						{
		...
		public static СправочникиВыборка.ДоговорыКонтрагентов Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld2200RRef [ВалютаВзаиморасчетов],_Fld2201RRef [ВедениеВзаиморасчетов],_Fld2202 [Комментарий],_Fld2203RRef [ВидУсловийДоговора],_Fld2204 [ДержатьРезервБезОплатыОграниченноеВремя],_Fld2205 [ДопустимаяСуммаЗадолженности],_Fld2206 [ДопустимоеЧислоДнейЗадолженности],_Fld2207RRef [Организация],_Fld2208 [КонтролироватьСуммуЗадолженности],_Fld2209 [КонтролироватьЧислоДнейЗадолженности],_Fld2210 [ОбособленныйУчетТоваровПоЗаказамПокупателей],_Fld2211 [ПроцентКомиссионногоВознаграждения],_Fld2212RRef [СпособРасчетаКомиссионногоВознаграждения],_Fld2213 [ПроцентПредоплаты],_Fld2214RRef [ТипЦен],_Fld2215RRef [ВидДоговора],_Fld2216 [ЧислоДнейРезерваБезОплаты],_Fld2217 [УчетАгентскогоНДС],_Fld2218RRef [ВидАгентскогоДоговора],_Fld2219 [РасчетыВУсловныхЕдиницах],_Fld2220 [КонтролироватьДенежныеСредстваКомитента],_Fld2221 [РеализацияНаЭкспорт],_Fld2222 [Дата],_Fld2223 [Номер],_Fld2224RRef [ВидВзаиморасчетов],_Fld2225 [ВестиПоДокументамРасчетовСКонтрагентом],_Fld2226RRef [ОсновнойПроект],_Fld2227RRef [ОсновнаяСтатьяДвиженияДенежныхСредств],_Fld2228 [СрокДействия],_Fld2229RRef [НаименованиеДляСчетаФактурыНаАванс],_Fld2230RRef [ПорядокРегистрацииСчетовФактурНаАвансПоДоговору],_Fld2231 [НалоговыйАгентПоОплате],_Fld2232 [УстановленСрокОплатыДляРезервовПоСомнительнымДолгам],_Fld2233 [СрокОплатыДляРезервовПоСомнительнымДолгам]
		 From _Reference86(NOLOCK)";
