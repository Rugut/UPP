...
namespace V82.Справочники//Менеджер
{
	...
	public partial class БанковскиеСчета:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.БанковскиеСчета НайтиПоКоду(decimal Код)
		{
		...
		public static СправочникиВыборка.БанковскиеСчета Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld1854 [НомерСчета],_Fld1855RRef [Банк],_Fld1856RRef [БанкДляРасчетов],_Fld1857 [ТекстКорреспондента],_Fld1858 [ТекстНазначения],_Fld1859 [ВидСчета],_Fld1860RRef [ВалютаДенежныхСредств],_Fld1861 [НомерИДатаРазрешения],_Fld1862 [ДатаОткрытия],_Fld1863 [ДатаЗакрытия],_Fld1864 [МесяцПрописью],_Fld1865 [СуммаБезКопеек]
		 From _Reference36(NOLOCK)";
