				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.ОсновноеИзображение = V82.СправочникиСсылка.ХранилищеДополнительнойИнформации.ВзятьИзКэша((byte[])Читалка.GetValue(13));
							Ссылка.МестоРожденияКодПоОКАТО = Читалка.GetString(14);
							Ссылка.ГруппаДоступаФизическогоЛица = V82.СправочникиСсылка.ГруппыДоступаФизическихЛиц.ВзятьИзКэша((byte[])Читалка.GetValue(15));
							...
							Ссылка.ИмеетНаучныеТруды = ((byte[])Читалка.GetValue(17))[0]==1;
							Ссылка.ИмеетИзобретения = ((byte[])Читалка.GetValue(18))[0]==1;
							return Ссылка;
						}
						else
						{
