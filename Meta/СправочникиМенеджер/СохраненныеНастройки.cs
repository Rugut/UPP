		...
		public static СправочникиВыборка.СохраненныеНастройки Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_Description [Наименование],_Fld3564RRef [ТипНастройки],_Fld3565 [НастраиваемыйОбъект],_Fld3566 [СохранятьАвтоматически],_Fld3567 [ИспользоватьПриОткрытии],_Fld3568 [СохранятьПериод],_Fld3569 [ОткрыватьПриЗапуске],_Fld3570 [ХранилищеНастроек],_Fld3571 [Описание],_Fld3572 [КраткийСоставНастройки]
		 From _Reference238(NOLOCK)";
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						while (Читалка.Read())
						{
							...
							Ссылка.СохранятьАвтоматически = ((byte[])Читалка.GetValue(7))[0]==1?true:false;
							Ссылка.ИспользоватьПриОткрытии = ((byte[])Читалка.GetValue(8))[0]==1?true:false;
							Ссылка.СохранятьПериод = ((byte[])Читалка.GetValue(9))[0]==1?true:false;
							Ссылка.ОткрыватьПриЗапуске = ((byte[])Читалка.GetValue(10))[0]==1?true:false;
							Ссылка.Описание = Читалка.GetString(12);
							Ссылка.КраткийСоставНастройки = Читалка.GetString(13);
							Выборка.Add(Ссылка);
						}
							return Выборка;
					}
