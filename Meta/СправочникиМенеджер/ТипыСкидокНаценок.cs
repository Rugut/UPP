...
namespace V82.Справочники//Менеджер
{
	...
	public partial class ТипыСкидокНаценок:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.ТипыСкидокНаценок НайтиПоКоду(decimal Код)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.ОбщееВремяНачала = Читалка.GetDateTime(10);
							Ссылка.ОбщееВремяОкончания = Читалка.GetDateTime(11);
							Ссылка.ОграничениеСкидкиНаценки = Читалка.GetDecimal(12);
							Ссылка.Условие = V82.Перечисления/*Ссылка*/.УсловияСкидкиНаценки.ПустаяСсылка.Получить((byte[])Читалка.GetValue(13));
							//Ссылка.Качество = new V82.СправочникиСсылка.Качество((byte[])Читалка.GetValue(14));
							Ссылка.ДляВсехПолучателей = ((byte[])Читалка.GetValue(15))[0]==1?true:false;
							Ссылка.ПоДнямНедели = ((byte[])Читалка.GetValue(16))[0]==1?true:false;
							Ссылка.ВидСкидки = V82.Перечисления/*Ссылка*/.ВидыСкидок.ПустаяСсылка.Получить((byte[])Читалка.GetValue(17));
							return Ссылка;
						}
						else
						{
		...
		public static СправочникиВыборка.ТипыСкидокНаценок Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld3946RRef [Валюта],_Fld3947 [ДляВсейНоменклатуры],_Fld3948 [ПроцентСкидкиНаценки],_Fld3949 [ЗначениеУсловия],_Fld3950 [ОбщееВремяНачала],_Fld3951 [ОбщееВремяОкончания],_Fld3952 [ОграничениеСкидкиНаценки],_Fld3953RRef [Условие],_Fld3954RRef [Качество],_Fld3955 [ДляВсехПолучателей],_Fld3956 [ПоДнямНедели],_Fld3957RRef [ВидСкидки]
		 From _Reference268(NOLOCK)";
