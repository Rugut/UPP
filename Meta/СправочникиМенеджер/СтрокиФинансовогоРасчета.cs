...
namespace V82.Справочники//Менеджер
{
	...
	public partial class СтрокиФинансовогоРасчета:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.СтрокиФинансовогоРасчета НайтиПоКоду(decimal Код)
		{
		...
		public static СправочникиВыборка.СтрокиФинансовогоРасчета Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld3852RRef [ВидСтроки],_Fld3853RRef [Измерение],_Fld3854 [Формула]
		 From _Reference253(NOLOCK)";
