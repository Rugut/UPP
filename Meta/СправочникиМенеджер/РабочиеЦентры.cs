...
namespace V82.Справочники//Менеджер
{
	...
	public partial class РабочиеЦентры:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.РабочиеЦентры НайтиПоКоду(decimal Код)
		{
		...
		public static СправочникиВыборка.РабочиеЦентры Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld3324RRef [Подразделение],_Fld3325 [Описание],_Fld3326 [ТребуетсяЗагрузкаПодчиненныхРабочихЦентров]
		 From _Reference214(NOLOCK)";
