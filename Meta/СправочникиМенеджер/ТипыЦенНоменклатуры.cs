...
namespace V82.Справочники//Менеджер
{
	...
	public partial class ТипыЦенНоменклатуры:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.ТипыЦенНоменклатуры НайтиПоКоду(decimal Код)
		{
		...
		public static СправочникиВыборка.ТипыЦенНоменклатуры Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld3965RRef [ВалютаЦены],_Fld3966RRef [БазовыйТипЦен],_Fld3967 [Рассчитывается],_Fld3968 [ПроцентСкидкиНаценки],_Fld3969 [ЦенаВключаетНДС],_Fld3970RRef [ПорядокОкругления],_Fld3971 [ОкруглятьВБольшуюСторону],_Fld3972 [Комментарий],_Fld3973RRef [СпособРасчетаЦены]
		 From _Reference269(NOLOCK)";
