...
namespace V82.Справочники//Менеджер
{
	...
	public partial class ТипыЦенНоменклатурыКонтрагентов:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.ТипыЦенНоменклатурыКонтрагентов НайтиПоКоду(decimal Код)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.ЦенаВключаетНДС = ((byte[])Читалка.GetValue(7))[0]==1?true:false;
							//Ссылка.ТипЦеныНоменклатуры = new V82.СправочникиСсылка.ТипыЦенНоменклатуры((byte[])Читалка.GetValue(8));
		...
		public static СправочникиВыборка.ТипыЦенНоменклатурыКонтрагентов Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld3977RRef [ВалютаЦены],_Fld3978 [ЦенаВключаетНДС],_Fld3979RRef [ТипЦеныНоменклатуры],_Fld3980 [ОписаниеТипаЦеныНоменклатурыКонтрагента],_Fld3981 [Комментарий]
		 From _Reference270(NOLOCK)";
