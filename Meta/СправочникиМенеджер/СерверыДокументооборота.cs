...
namespace V82.Справочники//Менеджер
{
	public partial class СерверыДокументооборота:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.СерверыДокументооборота НайтиПоКоду(decimal Код)
		{
		...
		public static СправочникиВыборка.СерверыДокументооборота Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld3392 [АдресЭлектроннойПочтыФНС],_Fld3393 [ДлительностьОжиданияСервера],_Fld3394 [Сертификат],_Fld3395 [АдресЭлектроннойПочтыПФР],_Fld3396 [АдресЭлектроннойПочтыФСГС]
		 From _Reference225(NOLOCK)";
