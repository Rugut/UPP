...
namespace V82.Справочники//Менеджер
{
	public partial class ЦиклыОбмена:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.ЦиклыОбмена НайтиПоКоду(decimal Код)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.ДатаСоздания = Читалка.GetDateTime(12);
							Ссылка.ДатаЗакрытия = Читалка.GetDateTime(13);
							Ссылка.ДатаПоследнегоСообщения = Читалка.GetDateTime(14);
							Ссылка.ДатаНачалаПериода = Читалка.GetDateTime(16);
							Ссылка.ДатаОкончанияПериода = Читалка.GetDateTime(17);
							Ссылка.ВидДокумента = Читалка.GetDecimal(18);
							Ссылка.ВидУслуги = V82.Перечисления/*Ссылка*/.ВидыУслугПриИОН.ПустаяСсылка.Получить((byte[])Читалка.GetValue(19));
							Ссылка.ФорматОтвета = V82.Перечисления/*Ссылка*/.ФорматОтветаНаЗапросИОН.ПустаяСсылка.Получить((byte[])Читалка.GetValue(20));
							Ссылка.Период = Читалка.GetString(21);
							Ссылка.Вид = V82.Перечисления/*Ссылка*/.ВидыЦикловОбмена.ПустаяСсылка.Получить((byte[])Читалка.GetValue(22));
							Ссылка.ФорматДокументооборота = V82.Перечисления/*Ссылка*/.ФорматыДокументооборотаСФНС.ПустаяСсылка.Получить((byte[])Читалка.GetValue(23));
							return Ссылка;
						}
						else
						{
		...
		public static СправочникиВыборка.ЦиклыОбмена Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld4228RRef [УчетнаяЗапись],_Fld4229RRef [Тип],_Fld4230 [Идентификатор],_Fld4231RRef [Предмет],_Fld4232RRef [Организация],_Fld4233RRef [ВнешняяОрганизация],_Fld4234 [ДатаСоздания],_Fld4235 [ДатаЗакрытия],_Fld4236 [ДатаПоследнегоСообщения],_Fld4237RRef [ВидОтчета],_Fld4238 [ДатаНачалаПериода],_Fld4239 [ДатаОкончанияПериода],_Fld4240 [ВидДокумента],_Fld4241RRef [ВидУслуги],_Fld4242RRef [ФорматОтвета],_Fld4243 [Период],_Fld4244RRef [Вид],_Fld4245RRef [ФорматДокументооборота]
		 From _Reference291(NOLOCK)";
