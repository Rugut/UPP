				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.Периодичность = V82.Перечисления/*Ссылка*/.Периодичность.ПустаяСсылка.Получить((byte[])Читалка.GetValue(12));
							Ссылка.ДатаИмпорта = Читалка.GetDateTime(13);
							Ссылка.ТипПолучателя = V82.Перечисления/*Ссылка*/.ТипыКонтролирующихОрганов.ПустаяСсылка.Получить((byte[])Читалка.GetValue(14));
							return Ссылка;
						}
						else
						{
		...
		public static СправочникиВыборка.ЭлектронныеПредставленияРегламентированныхОтчетов Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld4337RRef [Организация],_Fld4338RRef [Получатель],_Fld4339RRef [ВидОтчета],_Fld4340 [ДатаНачала],_Fld4341 [ДатаОкончания],_Fld4343 [Комментарий],_Fld4344RRef [Периодичность],_Fld4345 [ДатаИмпорта],_Fld4346RRef [ТипПолучателя]
		 From _Reference295(NOLOCK)";
