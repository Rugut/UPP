...
namespace V82.Справочники//Менеджер
{
	public partial class Пользователи:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.Пользователи НайтиПоКоду(decimal Код)
		{
		...
		public static СправочникиВыборка.Пользователи Выбрать()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					,_IsMetadata [Предопределенный]
					,_Code [Код]
					,_Description [Наименование],_Fld3235RRef [ПрофильПолномочийПользователя],_Fld3236RRef [ФизЛицо],_Fld3237 [ИдентификаторПользователяИБ]
		 From _Reference195(NOLOCK)";
