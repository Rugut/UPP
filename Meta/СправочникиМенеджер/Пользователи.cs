...
namespace V82.Справочники//Менеджер
{
	public partial class Пользователи:СправочникМенеджер
	{
		...
		public static СправочникиСсылка.Пользователи НайтиПоСсылке(Guid _Ссылка)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Ссылка.Родитель = V82.СправочникиСсылка.Пользователи.ВзятьИзКэша((byte[])Читалка.GetValue(4));
							Ссылка.ЭтоГруппа = ((byte[])Читалка.GetValue(5))[0]==0;
							Ссылка.Код = Читалка.GetString(6);
							Ссылка.Наименование = Читалка.GetString(7);
							if(!Ссылка.ЭтоГруппа)
							{
								Ссылка.ПрофильПолномочийПользователя = V82.СправочникиСсылка.ПрофилиПолномочийПользователей.ВзятьИзКэша((byte[])Читалка.GetValue(8));
								Ссылка.ФизЛицо = V82.СправочникиСсылка.ФизическиеЛица.ВзятьИзКэша((byte[])Читалка.GetValue(9));
							}
							return Ссылка;
						}
						else
		...
		public static СправочникиВыборка.Пользователи ИерархияВыбратьПоСсылке(Guid Родитель,int Режим,int Первые,Guid Мин,Guid Макс)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
		...
		public static СправочникиВыборка.Пользователи ИерархияСтраницаПоСсылке(Guid Родитель,int Режим,int Размер,int Номер)
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				Подключение.Open();
