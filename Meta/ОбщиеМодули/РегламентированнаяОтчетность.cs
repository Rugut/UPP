...
*/
		...
		public void роПроставитьНаВсехЛистах(/*Форма, Знач Значение, Знач ПрефиксНаЛисте, ЛистИсключение = Неопределено, ТолькоДляЛиста = Неопределено, ТолькоАктивныйЛистМногостраничнойчасти = Ложь, ИспользоватьНомер = Истина, ЛистыМногостраничные = Истина*/)
		{
			...
			{
				if(true/*ЭтоПБОЮЛ(Форма.Организация)*/)
				{
					/*Значение = "         ";*/
				}
				/*Исключение
...
*/
			/*// не возникала ошибка
*/
			if(true/*(Область.Защита)
		 Или (Найти(Область.Имя, "R") >0)
		 Или (Найти(Область.Имя, "C") >0)*/)
			{
...
*/
		...
		ОбъектНайденныйЭлемент.Заблокировать();*/
			if(true/*НЕ ЗарегистрироватьВнешнийОтчет(ОбъектНайденныйЭлемент, Объект)*/)
			{
			}
			/*// пытаемся записать
*/
...
*/
				...
				ВерсияФормыДанных = Форма.мСохраненныйДок.ДанныеОтчета.Получить().ВерсияФормы;*/
					if(true/*НЕ ПродолжитьПриНесоответствииВерсийФорм(ВерсияФормыДанных, ВыбФормаОтчета.мВерсияФормы)*/)
					{
					}
					/*Исключение
			Конецпопытки;*/
		...
		ЗаписатьСтатусОтчета(Форма.мСохраненныйДок.Ссылка, Форма.ЭлементыФормы.Статус.Значение);*/
			...
			/*Исключение
	КонецПопытки;*/
			if(true/*Форма.мЗаписываетсяНовыйДокумент = Истина*/)
			{
...
*/
	...
	КонецПопытки;*/
		...
		ПолеТД.ТекущаяОбласть = ПолеТД.Области.Найти(Ячейка.ИмяЯчейки);*/
			...
			if(true/*(Ячейка.СтрокаПП <> Неопределено) И (НЕ (ПустаяСтрока(Ячейка.СтрокаПП)))*/)
			{
				/*Попытка
...
*/
		...
		{
			if(true/*ТипЗнч(Доки) <> Тип("СписокЗначений") ИЛИ Доки.Количество() = 0*/)
			{
			}
			/*НовыйДок = Документы.ВыгрузкаРегламентированныхОтчетов.СоздатьДокумент();*/
...
*/
		...
		ОбъектЧтениеXML.ОткрытьФайл(ФайлВыгрузки);*/
			...
			while(true/*ОбъектЧтениеXML.Прочитать()*/)
			{
				if(true/*ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ОбъектЧтениеXML.Имя <> "СодПерСвед"*/)
				{
					while(true/*ОбъектЧтениеXML.ПрочитатьАтрибут()*/)
					{
		...
		Текст = Новый ЧтениеТекста;*/
			...
			while(true/*ТекСтр <> Неопределено*/)
			{
				/*ТекСтр = СокрЛП(ТекСтр);*/
				/*ВхождениеДвоеточия = Найти(ТекСтр, ":");*/
				if(true/*ВхождениеДвоеточия <> 0*/)
				{
...
*/
		...
		objShell = Новый COMОбъект("Shell.Application");*/
			...
			if(true/*Прав(ПутьККаталогуШрифтов, 1) <> "\"*/)
			{
				/*ПутьККаталогуШрифтов = ПутьККаталогуШрифтов + "\";*/
			}
			/*ФайлШрифтаСуществует = Ложь;*/
			if(true/*НЕ ФайлШрифтаСуществует*/)
			{
...
*/
	...
	КонецПопытки;*/
			...
			/*Попытка
		РеглОтчеты("РегламентированныйОтчет4ФСС").ПолучитьМакет("КомпонентаОбмена").Записать(ВремФайлКомпоненты);*/
			/*Исключение
		Сообщить("Не удалось выгрузить внешнюю компоненту ExtIntgr.dll в каталог программы:
				|" + ИнформацияОбОшибке().Описание, СтатусСообщения.Важное);*/
			/*КонецПопытки;*/
