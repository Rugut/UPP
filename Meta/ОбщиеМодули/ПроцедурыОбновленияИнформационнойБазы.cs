	...
	|	Ссылка";*/
		...
		Попытка
			ВРОбъект.Записать();*/
				/*Исключение
		#Если ТолстыйКлиентОбычноеПриложение Тогда
			Сообщить("Вид расчета " + ВРОбъект.Наименование + ": не удалось включить в расчетную базу новый в.р. """ + ОбщегоНазначения.ПолучитьПредставленияОбъектов(ВыборкаЗапроса.НовыйВидРасчета) + """!", СтатусСообщения.Важное);*/
				/*#КонецЕсли
		КонецПопытки;*/
			}
			/*; 
