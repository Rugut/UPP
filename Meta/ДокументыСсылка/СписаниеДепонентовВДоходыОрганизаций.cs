...
namespace V82.ДокументыСсылка
{
	[ProtoContract]
	[DataContract]
	public partial class СписаниеДепонентовВДоходыОрганизаций:ДокументСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		public static readonly Guid ГуидКласса = new Guid("4a5cec84-ca1e-4cef-9365-24f38ce65c04");
		...
		public СписаниеДепонентовВДоходыОрганизаций()
		{
		}
		
		public СписаниеДепонентовВДоходыОрганизаций(byte[] УникальныйИдентификатор)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							КраткийСоставДокумента = Читалка.GetString(17);
							//return Ссылка;
						}
						else
						{
		...
		public V82.ДокументыОбъект.СписаниеДепонентовВДоходыОрганизаций  ПолучитьОбъект()
		{
		...
		public static V82.ДокументыСсылка.СписаниеДепонентовВДоходыОрганизаций ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.ДокументыСсылка.СписаниеДепонентовВДоходыОрганизаций)Кэш[УИ];
			}
			var Ссылка = new V82.ДокументыСсылка.СписаниеДепонентовВДоходыОрганизаций(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
