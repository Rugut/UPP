...
namespace V82.ДокументыСсылка
{
	...
	public partial class Сторнирование:ДокументСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		///(Общ)
		///</summary>
		public object СторнируемыйДокумент {get;set;}//Сторнируемый документ
		///<summary>
		...
		public Сторнирование()
		{
		}
		
		public Сторнирование(byte[] УникальныйИдентификатор)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							ПериодРегистрации = Читалка.GetDateTime(9);
							//return Ссылка;
						}
						else
						{
		...
		public V82.ДокументыОбъект.Сторнирование  ПолучитьОбъект()
		{
		...
		public static V82.ДокументыСсылка.Сторнирование ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.ДокументыСсылка.Сторнирование)Кэш[УИ];
			}
			var Ссылка = new V82.ДокументыСсылка.Сторнирование(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
