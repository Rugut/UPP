...
namespace V82.ДокументыСсылка
{
	...
	public partial class ПереоценкаТоваровВРознице:ДокументСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public ПереоценкаТоваровВРознице()
		{
		}
		
		public ПереоценкаТоваровВРознице(byte[] УникальныйИдентификатор)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							ВидОперации = V82.Перечисления/*Ссылка*/.ВидыОперацийПереоценкаТоваровВРознице.ПустаяСсылка.Получить((byte[])Читалка.GetValue(9));
							ДокументУстановкаЦен = new V82.ДокументыСсылка.УстановкаЦенНоменклатуры((byte[])Читалка.GetValue(10));
							//return Ссылка;
						}
						else
						{
		...
		public V82.ДокументыОбъект.ПереоценкаТоваровВРознице  ПолучитьОбъект()
		{
			...
			Объект.ДокументУстановкаЦен = ДокументУстановкаЦен;
			return Объект;
		}
		
		private static readonly Hashtable Кэш = new Hashtable(1000);
		
		public static V82.ДокументыСсылка.ПереоценкаТоваровВРознице ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.ДокументыСсылка.ПереоценкаТоваровВРознице)Кэш[УИ];
			}
			var Ссылка = new V82.ДокументыСсылка.ПереоценкаТоваровВРознице(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
