...
namespace V82.ДокументыСсылка
{
	...
	public partial class ОплатаСверхурочныхЧасов:ДокументСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public V82.ДокументыСсылка.ОплатаСверхурочныхЧасов ПерерассчитываемыйДокумент {get;set;}//Перерассчитываемый документ
		
		public ОплатаСверхурочныхЧасов()
		{
		}
		
		public ОплатаСверхурочныхЧасов(byte[] УникальныйИдентификатор)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							ПерерассчитываемыйДокумент = new V82.ДокументыСсылка.ОплатаСверхурочныхЧасов((byte[])Читалка.GetValue(9));
							//return Ссылка;
						}
						else
						{
		...
		public V82.ДокументыОбъект.ОплатаСверхурочныхЧасов  ПолучитьОбъект()
		{
		...
		public static V82.ДокументыСсылка.ОплатаСверхурочныхЧасов ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.ДокументыСсылка.ОплатаСверхурочныхЧасов)Кэш[УИ];
			}
			var Ссылка = new V82.ДокументыСсылка.ОплатаСверхурочныхЧасов(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
