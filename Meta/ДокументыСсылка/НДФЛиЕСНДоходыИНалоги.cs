...
namespace V82.ДокументыСсылка
{
	...
	public partial class НДФЛиЕСНДоходыИНалоги:ДокументСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public НДФЛиЕСНДоходыИНалоги()
		{
		}
		
		public НДФЛиЕСНДоходыИНалоги(byte[] УникальныйИдентификатор)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							НалоговыйПериод = Читалка.GetDecimal(8);
							//return Ссылка;
						}
						else
						{
		...
		public V82.ДокументыОбъект.НДФЛиЕСНДоходыИНалоги  ПолучитьОбъект()
		{
		...
		public static V82.ДокументыСсылка.НДФЛиЕСНДоходыИНалоги ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.ДокументыСсылка.НДФЛиЕСНДоходыИНалоги)Кэш[УИ];
			}
			var Ссылка = new V82.ДокументыСсылка.НДФЛиЕСНДоходыИНалоги(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
