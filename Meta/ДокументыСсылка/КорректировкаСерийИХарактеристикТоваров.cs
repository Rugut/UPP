...
namespace V82.ДокументыСсылка
{
	...
	public partial class КорректировкаСерийИХарактеристикТоваров:ДокументСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public КорректировкаСерийИХарактеристикТоваров()
		{
		}
		
		public КорректировкаСерийИХарактеристикТоваров(byte[] УникальныйИдентификатор)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							ВидОперации = V82.Перечисления/*Ссылка*/.ВидыОперацийКорректировкаСерийИХарактеристикТоваров.ПустаяСсылка.Получить((byte[])Читалка.GetValue(12));
							//return Ссылка;
						}
						else
						{
		...
		public V82.ДокументыОбъект.КорректировкаСерийИХарактеристикТоваров  ПолучитьОбъект()
		{
			var Объект = new V82.ДокументыОбъект.КорректировкаСерийИХарактеристикТоваров();
		...
		public static V82.ДокументыСсылка.КорректировкаСерийИХарактеристикТоваров ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.ДокументыСсылка.КорректировкаСерийИХарактеристикТоваров)Кэш[УИ];
			}
			var Ссылка = new V82.ДокументыСсылка.КорректировкаСерийИХарактеристикТоваров(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
