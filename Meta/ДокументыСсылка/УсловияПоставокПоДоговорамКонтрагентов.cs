...
namespace V82.ДокументыСсылка
{
	...
	public partial class УсловияПоставокПоДоговорамКонтрагентов:ДокументСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public decimal/*(15.2)*/ СуммаУсловийДоговора {get;set;}//Сумма условий договора
		public string/*(0)*/ Комментарий {get;set;}
		public V82.СправочникиСсылка.Пользователи Ответственный {get;set;}
		
		public УсловияПоставокПоДоговорамКонтрагентов()
		{
		}
		
		public УсловияПоставокПоДоговорамКонтрагентов(byte[] УникальныйИдентификатор)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							Периодичность = V82.Перечисления/*Ссылка*/.Периодичность.ПустаяСсылка.Получить((byte[])Читалка.GetValue(8));
							СуммаУсловийДоговора = Читалка.GetDecimal(9);
		...
		public V82.ДокументыОбъект.УсловияПоставокПоДоговорамКонтрагентов  ПолучитьОбъект()
		{
			var Объект = new V82.ДокументыОбъект.УсловияПоставокПоДоговорамКонтрагентов();
			...
			Объект.СуммаУсловийДоговора = СуммаУсловийДоговора;
		...
		public static V82.ДокументыСсылка.УсловияПоставокПоДоговорамКонтрагентов ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.ДокументыСсылка.УсловияПоставокПоДоговорамКонтрагентов)Кэш[УИ];
			}
			var Ссылка = new V82.ДокументыСсылка.УсловияПоставокПоДоговорамКонтрагентов(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
