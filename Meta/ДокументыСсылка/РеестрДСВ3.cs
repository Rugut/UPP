...
namespace V82.ДокументыСсылка
{
	...
	public partial class РеестрДСВ3:ДокументСсылка,IСериализаторProtoBuf,IСериализаторJson
	{
		...
		public РеестрДСВ3(byte[] УникальныйИдентификатор)
		{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					using (var Читалка = Команда.ExecuteReader())
					{
						if (Читалка.Read())
						{
							...
							НомерПлатежногоПоручения = Читалка.GetString(10);
							ДатаПлатежногоПоручения = Читалка.GetDateTime(11);
							ДатаИсполненияПлатежногоПоручения = Читалка.GetDateTime(12);
		...
		public V82.ДокументыОбъект.РеестрДСВ3  ПолучитьОбъект()
		{
			...
			Объект.НомерПлатежногоПоручения = НомерПлатежногоПоручения;
			Объект.ДатаПлатежногоПоручения = ДатаПлатежногоПоручения;
			Объект.ДатаИсполненияПлатежногоПоручения = ДатаИсполненияПлатежногоПоручения;
		...
		public static V82.ДокументыСсылка.РеестрДСВ3 ВзятьИзКэша(byte[] УникальныйИдентификатор)
		{
			var УИ = new Guid(УникальныйИдентификатор);
			if (Кэш.ContainsKey(УИ))
			{
				return (V82.ДокументыСсылка.РеестрДСВ3)Кэш[УИ];
			}
			var Ссылка = new V82.ДокументыСсылка.РеестрДСВ3(УникальныйИдентификатор);
			Кэш.Add(УИ, Ссылка);
			return Ссылка;
		}
