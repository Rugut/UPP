	...
	public partial class НормативыОбслуживанияОС: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			for (var НомерЭлемента = 1;НомерЭлемента < 1000;НомерЭлемента++)
			{
				...
				Объект.Услуга = V82.Справочники/*Менеджер*/.Номенклатура.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.Номенклатура();
				...
				Объект.ПараметрВыработки = V82.Справочники/*Менеджер*/.ПараметрыВыработкиОС.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.ПараметрыВыработкиОС();
				Объект.ЗначениеПараметраВыработки = ГенераторСлучайныхЧисел.Следующий(99999999);
