...
{
	public partial class ЦиклыОбмена: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			for (var НомерЭлемента = 1;НомерЭлемента < 1000;НомерЭлемента++)
			{
				...
				Объект.УчетнаяЗапись = V82.Справочники/*Менеджер*/.УчетныеЗаписиДокументооборота.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.УчетныеЗаписиДокументооборота();
				...
				Объект.ВидДокумента = ГенераторСлучайныхЧисел.Следующий(99);
				...
				Объект.Период = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 16);
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.ЦиклыОбмена.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника ЦиклыОбмена. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника ЦиклыОбмена. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.ЦиклыОбмена.Выбрать())
			{
				НомерЭлемента++;
