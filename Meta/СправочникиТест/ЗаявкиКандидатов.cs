...
namespace V82.СправочникиТест
{
	...
	///(Упр)
	///</summary>
	public partial class ЗаявкиКандидатов: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			for (var НомерЭлемента = 1;НомерЭлемента < 1000;НомерЭлемента++)
			{
				...
				Объект.Состояние = V82.Справочники/*Менеджер*/.СостоянияЗаявокКандидатов.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.СостоянияЗаявокКандидатов();
				...
				Объект.РезультатЗакрытия = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 255);
				Объект.ГруппаЗаявок = V82.Справочники/*Менеджер*/.ГруппыЗаявокКандидатов.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.ГруппыЗаявокКандидатов();
				Объект.ДатаОткрытия = ГенераторСлучайныхЧисел.СледующаяДата();
				Объект.ИсточникИнформации = V82.Справочники/*Менеджер*/.ИсточникиИнформации.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.ИсточникиИнформации();
				Объект.Вакансия = V82.Справочники/*Менеджер*/.Вакансии.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.Вакансии();
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.ЗаявкиКандидатов.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника ЗаявкиКандидатов. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника ЗаявкиКандидатов. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.ЗаявкиКандидатов.Выбрать())
			{
				НомерЭлемента++;
