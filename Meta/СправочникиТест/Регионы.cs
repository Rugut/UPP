...
namespace V82.СправочникиТест
{
	...
	///Регионы
	///</summary>
	public partial class Регионы: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			{
				var Объект = V82.Справочники/*Менеджер*/.Регионы.СоздатьЭлемент();
				...
				Объект.КодРегиона = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 5);
				Объект.КодАдресногоЭлемента = ГенераторСлучайныхЧисел.Следующий(99999999);
				Объект.ЖДСтанцияНазначения = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 100);
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.Регионы.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника Регионы. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника Регионы. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.Регионы.Выбрать())
			{
				НомерЭлемента++;
