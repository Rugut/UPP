...
namespace V82.СправочникиТест
{
	...
	///(Упр)
	///</summary>
	public partial class НоменклатурныеГруппы: СправочникТест
	{
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.НоменклатурныеГруппы.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника НоменклатурныеГруппы. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника НоменклатурныеГруппы. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.НоменклатурныеГруппы.Выбрать())
			{
				НомерЭлемента++;
