...
{
	public partial class ТарифыБанковНаДенежныеПереводы: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			for (var НомерЭлемента = 1;НомерЭлемента < 1000;НомерЭлемента++)
			{
				...
				Объект.МинимальныйТариф = ГенераторСлучайныхЧисел.Следующий(99999999);
				Объект.МаксимальныйТариф = ГенераторСлучайныхЧисел.Следующий(99999999);
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.ТарифыБанковНаДенежныеПереводы.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника ТарифыБанковНаДенежныеПереводы. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника ТарифыБанковНаДенежныеПереводы. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.ТарифыБанковНаДенежныеПереводы.Выбрать())
			{
				НомерЭлемента++;
