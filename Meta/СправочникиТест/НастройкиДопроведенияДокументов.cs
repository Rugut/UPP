...
{
	public partial class НастройкиДопроведенияДокументов: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			for (var НомерЭлемента = 1;НомерЭлемента < 1000;НомерЭлемента++)
			{
				...
				Объект.НомерДняКонецЗапуска = ГенераторСлучайныхЧисел.Следующий(9);
				Объект.НомерДняНачалоЗапуска = ГенераторСлучайныхЧисел.Следующий(9);
				...
				Объект.КоличествоПериодовОтставанияКонецИнтервала = ГенераторСлучайныхЧисел.Следующий(9);
				Объект.КоличествоПериодовОтставанияНачалоИнтервала = ГенераторСлучайныхЧисел.Следующий(9);
				...
				Объект.РассчитыватьКонецИнтервала = ГенераторСлучайныхЧисел.СледующееБулево();
				Объект.РассчитыватьНачалоИнтервала = ГенераторСлучайныхЧисел.СледующееБулево();
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.НастройкиДопроведенияДокументов.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника НастройкиДопроведенияДокументов. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника НастройкиДопроведенияДокументов. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.НастройкиДопроведенияДокументов.Выбрать())
			{
				НомерЭлемента++;
