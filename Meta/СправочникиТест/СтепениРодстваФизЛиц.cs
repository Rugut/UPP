...
namespace V82.СправочникиТест
{
	...
	///(Общ)
	///</summary>
	public partial class СтепениРодстваФизЛиц: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("Создание 1000 элементов справочника СтепениРодстваФизЛиц. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			for (var НомерЭлемента = 1;НомерЭлемента < 1000;НомерЭлемента++)
			{
				var Объект = V82.Справочники/*Менеджер*/.СтепениРодстваФизЛиц.СоздатьЭлемент();
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.СтепениРодстваФизЛиц.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника СтепениРодстваФизЛиц. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника СтепениРодстваФизЛиц. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.СтепениРодстваФизЛиц.Выбрать())
			{
				НомерЭлемента++;
