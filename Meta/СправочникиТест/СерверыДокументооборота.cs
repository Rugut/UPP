...
{
	public partial class СерверыДокументооборота: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			for (var НомерЭлемента = 1;НомерЭлемента < 1000;НомерЭлемента++)
			{
				...
				Объект.АдресЭлектроннойПочтыФНС = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 255);
				Объект.ДлительностьОжиданияСервера = ГенераторСлучайныхЧисел.Следующий(9999);
				Объект.Сертификат = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 40);
				Объект.АдресЭлектроннойПочтыПФР = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 255);
				Объект.АдресЭлектроннойПочтыФСГС = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 255);
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.СерверыДокументооборота.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника СерверыДокументооборота. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника СерверыДокументооборота. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.СерверыДокументооборота.Выбрать())
			{
				НомерЭлемента++;
