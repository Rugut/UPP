...
namespace V82.СправочникиТест
{
	...
	///(Общ)
	///</summary>
	public partial class СтатьиЗатрат: СправочникТест
	{
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.СтатьиЗатрат.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника СтатьиЗатрат. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника СтатьиЗатрат. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.СтатьиЗатрат.Выбрать())
			{
				НомерЭлемента++;
