...
{
	public partial class ВидыСчетчиковДляЗначенийРазрядов: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			for (var НомерЭлемента = 1;НомерЭлемента < 1000;НомерЭлемента++)
			{
				...
				Объект.РазмерСчетчика = ГенераторСлучайныхЧисел.Следующий(9);
				...
				Объект.НачальноеЗначение = ГенераторСлучайныхЧисел.Следующий(99999999);
				Объект.КонечноеЗначение = ГенераторСлучайныхЧисел.Следующий(99999999);
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.ВидыСчетчиковДляЗначенийРазрядов.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника ВидыСчетчиковДляЗначенийРазрядов. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника ВидыСчетчиковДляЗначенийРазрядов. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.ВидыСчетчиковДляЗначенийРазрядов.Выбрать())
			{
				НомерЭлемента++;
