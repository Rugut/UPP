...
namespace V82.СправочникиТест
{
	...
	///</summary>
	public partial class НастройкиЗакрытияМесяца: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			for (var НомерЭлемента = 1;НомерЭлемента < 1000;НомерЭлемента++)
			{
				...
				Объект.ВариантНастройки = ГенераторСлучайныхЧисел.Следующий();
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.НастройкиЗакрытияМесяца.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника НастройкиЗакрытияМесяца. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника НастройкиЗакрытияМесяца. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.НастройкиЗакрытияМесяца.Выбрать())
			{
				НомерЭлемента++;
