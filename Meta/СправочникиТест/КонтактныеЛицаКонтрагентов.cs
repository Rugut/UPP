...
namespace V82.СправочникиТест
{
	...
	///</summary>
	public partial class КонтактныеЛицаКонтрагентов: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			for (var НомерЭлемента = 1;НомерЭлемента < 1000;НомерЭлемента++)
			{
				...
				Объект.Должность = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 100);
				Объект.Комментарий = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 255);
				Объект.КонтактноеЛицо = V82.Справочники/*Менеджер*/.КонтактныеЛица.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.КонтактныеЛица();
				Объект.РольКонтактногоЛица = V82.Справочники/*Менеджер*/.РолиКонтактныхЛиц.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.РолиКонтактныхЛиц();
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.КонтактныеЛицаКонтрагентов.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника КонтактныеЛицаКонтрагентов. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника КонтактныеЛицаКонтрагентов. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.КонтактныеЛицаКонтрагентов.Выбрать())
			{
				НомерЭлемента++;
