	...
	public partial class ДоговорыКонтрагентов: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			for (var НомерЭлемента = 1;НомерЭлемента < 1000;НомерЭлемента++)
			{
				...
				Объект.ВалютаВзаиморасчетов = V82.Справочники/*Менеджер*/.Валюты.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(99)) ?? new V82.СправочникиСсылка.Валюты();
				...
				Объект.ДопустимаяСуммаЗадолженности = ГенераторСлучайныхЧисел.Следующий(99999999);
				Объект.ДопустимоеЧислоДнейЗадолженности = ГенераторСлучайныхЧисел.Следующий(99999999);
				...
				Объект.ПроцентКомиссионногоВознаграждения = ГенераторСлучайныхЧисел.Следующий(9999);
				...
				Объект.ПроцентПредоплаты = ГенераторСлучайныхЧисел.Следующий(9999);
				...
				Объект.ЧислоДнейРезерваБезОплаты = ГенераторСлучайныхЧисел.Следующий(99999999);
				...
				Объект.Номер = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 20);
				Объект.ВидВзаиморасчетов = V82.Справочники/*Менеджер*/.ВидыВзаиморасчетов.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.ВидыВзаиморасчетов();
				Объект.ВестиПоДокументамРасчетовСКонтрагентом = ГенераторСлучайныхЧисел.СледующееБулево();
				Объект.ОсновнаяСтатьяДвиженияДенежныхСредств = V82.Справочники/*Менеджер*/.СтатьиДвиженияДенежныхСредств.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.СтатьиДвиженияДенежныхСредств();
				Объект.СрокДействия = ГенераторСлучайныхЧисел.СледующаяДата();
				Объект.НаименованиеДляСчетаФактурыНаАванс = V82.Справочники/*Менеджер*/.Номенклатура.НайтиПоКоду(ГенераторСлучайныхЧисел.Следующий(999)) ?? new V82.СправочникиСсылка.Номенклатура();
				...
				Объект.СрокОплатыДляРезервовПоСомнительнымДолгам = ГенераторСлучайныхЧисел.Следующий(9999);
