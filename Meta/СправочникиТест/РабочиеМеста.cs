...
{
	public partial class РабочиеМеста: СправочникТест
	{
		...
		public static void Создать1000()
		{
			...
			{
				var Объект = V82.Справочники/*Менеджер*/.РабочиеМеста.СоздатьЭлемент();
				...
				Объект.ИмяКомпьютера = ГенераторСлучайныхЧисел.СлучайноеРусскоеСлово(3, 50);
				Объект.СетевойПорт = ГенераторСлучайныхЧисел.Следующий(99999);
		...
		{
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.РабочиеМеста.Выбрать())
			{
		...
		public static void СериализацияProtoBuf1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияProtoBuf 1000 элементов справочника РабочиеМеста. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
		...
		public static void СериализацияJson1000()
		{
			var Старт = DateTime.Now;
			Console.WriteLine("--------------------------------------------------------------------");
			Console.WriteLine("СериализацияJson 1000 элементов справочника РабочиеМеста. Старт: " + Старт.ToString("dd.MM.yy HH:mm:ss.fff") + ".");
			var НомерЭлемента = 0;
			foreach (var Ссылка in V82.Справочники/*Менеджер*/.РабочиеМеста.Выбрать())
			{
				НомерЭлемента++;
