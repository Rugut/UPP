	...
	public partial class РезультатИспытательногоСрока:ДокументОбъект
	{
			...
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					else
					{
						...
						,_Number	= @Номер
						,_Fld16730	= @Комментарий
						,_Fld16731RRef	= @Ответственный
						,_Fld16732RRef	= @Сотрудник
						,_Fld16733RRef	= @ФизЛицо
						,_Fld16734RRef	= @Результат
						,_Fld16735	= @УдалитьКраткийСоставДокумента
						,_Fld16736	= @ДатаИзменения
					...
					Команда.Parameters.AddWithValue("Результат", Результат.Ключ());
					Команда.Parameters.AddWithValue("УдалитьКраткийСоставДокумента", УдалитьКраткийСоставДокумента);
		...
		public void Удалить()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					Команда.CommandText = @"Delete _Document580
					Where _IDRRef=@Ссылка";
					Команда.Parameters.AddWithValue("Ссылка", Ссылка.ToByteArray());
					Команда.ExecuteNonQuery();
				}
