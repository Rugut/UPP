	...
	public partial class ЗакрытиеГода:ДокументОбъект
	{
		...
		public void Записать()
		{
			...
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					...
					Команда.Parameters.AddWithValue("РеформацияБалансаБУ", РеформацияБалансаБУ);
					Команда.Parameters.AddWithValue("ЗакрытиеСчетовНУ", ЗакрытиеСчетовНУ);
		...
		public void Удалить()
		{
			using (var Подключение = new SqlConnection(СтрокаСоединения))
			{
				...
				using (var Команда = Подключение.CreateCommand())
				{
					Команда.CommandText = @"Delete _Document19657
					Where _IDRRef=@Ссылка";
					Команда.Parameters.AddWithValue("Ссылка", Ссылка.ToByteArray());
					Команда.ExecuteNonQuery();
				}
