...
namespace V82.Справочники
{
	...
	public partial class КлючиАналитикиВидаУчетаСервис : Service
	{
		...
		public object Get(КлючиАналитикиВидаУчетаНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.КлючиАналитикиВидаУчета.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new КлючиАналитикиВидаУчетаОтвет() { Ответ = "КлючиАналитикиВидаУчета c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(КлючиАналитикиВидаУчетаВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.КлючиАналитикиВидаУчета.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
