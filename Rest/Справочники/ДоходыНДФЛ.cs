...
namespace V82.Справочники
{
	...
	public partial class ДоходыНДФЛСервис : Service
	{
		...
		public object Get(ДоходыНДФЛНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ДоходыНДФЛ.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ДоходыНДФЛОтвет() { Ответ = "ДоходыНДФЛ c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ДоходыНДФЛВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.ДоходыНДФЛ.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
