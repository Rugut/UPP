...
namespace V82.Справочники
{
	...
	public partial class ШкалыОценокКомпетенцийСервис : Service
	{
		...
		public object Get(ШкалыОценокКомпетенцийНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ШкалыОценокКомпетенций.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ШкалыОценокКомпетенцийОтвет() { Ответ = "ШкалыОценокКомпетенций c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
