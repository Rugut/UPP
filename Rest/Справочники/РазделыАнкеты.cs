...
namespace V82.Справочники
{
	...
	public partial class РазделыАнкетыСервис : Service
	{
		...
		public object Get(РазделыАнкетыНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.РазделыАнкеты.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new РазделыАнкетыОтвет() { Ответ = "РазделыАнкеты c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(РазделыАнкетыВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.РазделыАнкеты.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
