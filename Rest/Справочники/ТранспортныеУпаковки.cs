...
namespace V82.Справочники
{
	...
	public partial class ТранспортныеУпаковкиСервис : Service
	{
		...
		public object Get(ТранспортныеУпаковкиНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ТранспортныеУпаковки.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ТранспортныеУпаковкиОтвет() { Ответ = "ТранспортныеУпаковки c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ТранспортныеУпаковкиВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.ТранспортныеУпаковки.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
