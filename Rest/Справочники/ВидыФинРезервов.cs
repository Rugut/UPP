...
namespace V82.Справочники
{
	...
	public partial class ВидыФинРезервовСервис : Service
	{
		...
		public object Get(ВидыФинРезервовНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ВидыФинРезервов.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ВидыФинРезервовОтвет() { Ответ = "ВидыФинРезервов c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ВидыФинРезервовВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.ВидыФинРезервов.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
