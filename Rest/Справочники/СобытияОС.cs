...
namespace V82.Справочники
{
	...
	public partial class СобытияОССервис : Service
	{
		
		public object Get(СобытияОСЗапрос Запрос)
		{
			return null;
		}
		...
		public object Get(СобытияОСНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.СобытияОС.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new СобытияОСОтвет() { Ответ = "СобытияОС c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(СобытияОСВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.СобытияОС.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
