...
namespace V82.Справочники
{
	...
	public partial class ФильтрыДляЭлектронныхПисемСервис : Service
	{
		...
		public object Get(ФильтрыДляЭлектронныхПисемНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ФильтрыДляЭлектронныхПисем.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ФильтрыДляЭлектронныхПисемОтвет() { Ответ = "ФильтрыДляЭлектронныхПисем c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
