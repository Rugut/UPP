...
namespace V82.Справочники
{
	...
	[Маршрут("Справочники/ПрофилиИзмененияПлановПоПериодам/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}")]
	[Маршрут("Справочники/ПрофилиИзмененияПлановПоПериодам/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}/{___Мин}")]
	[Маршрут("Справочники/ПрофилиИзмененияПлановПоПериодам/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}/{___Мин}/{___Макс}")]
	public class ПрофилиИзмененияПлановПоПериодамИерархияВыбратьПоСсылке: V82.СправочникиСсылка.ПрофилиИзмененияПлановПоПериодам,IReturn<ПрофилиИзмененияПлановПоПериодамИерархияВыбратьПоСсылке>
	{
	...
	[Маршрут("Справочники/ПрофилиИзмененияПлановПоПериодам/ИерархияСтраницаПоСсылке","{___Родитель}/{___Режим}/{___Размер}/{___Номер}")]
	public class ПрофилиИзмененияПлановПоПериодамИерархияСтраницаПоСсылке: V82.СправочникиСсылка.ПрофилиИзмененияПлановПоПериодам,IReturn<ПрофилиИзмененияПлановПоПериодамИерархияСтраницаПоСсылке>
	{
		public Guid ___Родитель {get; set;}
		public int ___Режим {get; set;}
		public int ___Размер {get; set;}
	...
	public partial class ПрофилиИзмененияПлановПоПериодамСервис : Service
	{
		...
		public object Get(ПрофилиИзмененияПлановПоПериодамНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ПрофилиИзмененияПлановПоПериодам.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ПрофилиИзмененияПлановПоПериодамОтвет() { Ответ = "ПрофилиИзмененияПлановПоПериодам c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ПрофилиИзмененияПлановПоПериодамИерархияВыбратьПоСсылке Запрос)
		{
			if (Запрос.___Макс == Guid.Empty)
			{
				Запрос.___Макс = Guid.Parse("ffffffff-ffff-ffff-ffff-ffffffffffff");
			...
			var Коллекция = new List<object>();
			foreach (var Ссылка in V82.Справочники.ПрофилиИзмененияПлановПоПериодам.ИерархияВыбратьПоСсылке(Запрос.___Родитель,Запрос.___Режим,Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
				Коллекция.Add(Ссылка.ПолучитьОбъект());
			}
			return Коллекция;
		...
		public object Get(ПрофилиИзмененияПлановПоПериодамИерархияСтраницаПоСсылке Запрос)
		{
			return null;
		}
