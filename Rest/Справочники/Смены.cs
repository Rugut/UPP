...
namespace V82.Справочники
{
	...
	public partial class СменыСервис : Service
	{
		...
		public object Get(СменыНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.Смены.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new СменыОтвет() { Ответ = "Смены c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(СменыВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.Смены.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
