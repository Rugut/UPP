...
namespace V82.Справочники
{
	...
	[Маршрут("Справочники/ШаблоныФормулРасчета/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}")]
	[Маршрут("Справочники/ШаблоныФормулРасчета/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}/{___Мин}")]
	[Маршрут("Справочники/ШаблоныФормулРасчета/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}/{___Мин}/{___Макс}")]
	public class ШаблоныФормулРасчетаИерархияВыбратьПоСсылке: V82.СправочникиСсылка.ШаблоныФормулРасчета,IReturn<ШаблоныФормулРасчетаИерархияВыбратьПоСсылке>
	{
	...
	[Маршрут("Справочники/ШаблоныФормулРасчета/ИерархияСтраницаПоСсылке","{___Родитель}/{___Режим}/{___Размер}/{___Номер}")]
	public class ШаблоныФормулРасчетаИерархияСтраницаПоСсылке: V82.СправочникиСсылка.ШаблоныФормулРасчета,IReturn<ШаблоныФормулРасчетаИерархияСтраницаПоСсылке>
	{
		public Guid ___Родитель {get; set;}
		public int ___Режим {get; set;}
		public int ___Размер {get; set;}
	...
	public partial class ШаблоныФормулРасчетаСервис : Service
	{
		...
		public object Get(ШаблоныФормулРасчетаНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ШаблоныФормулРасчета.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ШаблоныФормулРасчетаОтвет() { Ответ = "ШаблоныФормулРасчета c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ШаблоныФормулРасчетаВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.ШаблоныФормулРасчета.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
		...
		public object Get(ШаблоныФормулРасчетаИерархияВыбратьПоСсылке Запрос)
		{
			if (Запрос.___Макс == Guid.Empty)
			{
				Запрос.___Макс = Guid.Parse("ffffffff-ffff-ffff-ffff-ffffffffffff");
			...
			var Коллекция = new List<object>();
			foreach (var Ссылка in V82.Справочники.ШаблоныФормулРасчета.ИерархияВыбратьПоСсылке(Запрос.___Родитель,Запрос.___Режим,Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
				Коллекция.Add(Ссылка.ПолучитьОбъект());
			}
			return Коллекция;
		...
		public object Get(ШаблоныФормулРасчетаИерархияСтраницаПоСсылке Запрос)
		{
			return null;
		}
