...
namespace V82.Справочники
{
	...
	public partial class ПричиныЗакрытияЗаказовСервис : Service
	{
		
		public object Get(ПричиныЗакрытияЗаказовЗапрос Запрос)
		{
			return null;
		}
		...
		public object Get(ПричиныЗакрытияЗаказовНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ПричиныЗакрытияЗаказов.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ПричиныЗакрытияЗаказовОтвет() { Ответ = "ПричиныЗакрытияЗаказов c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ПричиныЗакрытияЗаказовВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.ПричиныЗакрытияЗаказов.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
