...
namespace V82.Справочники
{
	...
	public partial class РаботодателиСервис : Service
	{
		...
		public object Get(РаботодателиНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.Работодатели.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new РаботодателиОтвет() { Ответ = "Работодатели c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(РаботодателиВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.Работодатели.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
