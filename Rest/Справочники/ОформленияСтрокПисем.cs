...
namespace V82.Справочники
{
	...
	public partial class ОформленияСтрокПисемСервис : Service
	{
		...
		public object Get(ОформленияСтрокПисемНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ОформленияСтрокПисем.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ОформленияСтрокПисемОтвет() { Ответ = "ОформленияСтрокПисем c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
