...
namespace V82.Справочники
{
	...
	public partial class ОтраслевыеСегментыПоМСФОСервис : Service
	{
		...
		public object Get(ОтраслевыеСегментыПоМСФОНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ОтраслевыеСегментыПоМСФО.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ОтраслевыеСегментыПоМСФООтвет() { Ответ = "ОтраслевыеСегментыПоМСФО c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
