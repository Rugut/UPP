...
namespace V82.Справочники
{
	...
	public partial class РабочиеМестаСервис : Service
	{
		
		public object Get(РабочиеМестаЗапрос Запрос)
		{
			return null;
		}
		...
		public object Get(РабочиеМестаНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.РабочиеМеста.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new РабочиеМестаОтвет() { Ответ = "РабочиеМеста c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(РабочиеМестаВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.РабочиеМеста.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
