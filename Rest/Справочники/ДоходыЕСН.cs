...
namespace V82.Справочники
{
	...
	public partial class ДоходыЕСНСервис : Service
	{
		...
		public object Get(ДоходыЕСННайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ДоходыЕСН.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ДоходыЕСНОтвет() { Ответ = "ДоходыЕСН c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ДоходыЕСНВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.ДоходыЕСН.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
