...
namespace V82.Справочники
{
	...
	public partial class ЭДПрисоединенныеФайлыСервис : Service
	{
		...
		public object Get(ЭДПрисоединенныеФайлыНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ЭДПрисоединенныеФайлы.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ЭДПрисоединенныеФайлыОтвет() { Ответ = "ЭДПрисоединенныеФайлы c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ЭДПрисоединенныеФайлыВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.ЭДПрисоединенныеФайлы.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
