...
namespace V82.Справочники
{
	...
	[Маршрут("Справочники/РасходыБудущихПериодов/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}")]
	[Маршрут("Справочники/РасходыБудущихПериодов/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}/{___Мин}")]
	[Маршрут("Справочники/РасходыБудущихПериодов/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}/{___Мин}/{___Макс}")]
	public class РасходыБудущихПериодовИерархияВыбратьПоСсылке: V82.СправочникиСсылка.РасходыБудущихПериодов,IReturn<РасходыБудущихПериодовИерархияВыбратьПоСсылке>
	{
	...
	[Маршрут("Справочники/РасходыБудущихПериодов/ИерархияСтраницаПоСсылке","{___Родитель}/{___Режим}/{___Размер}/{___Номер}")]
	public class РасходыБудущихПериодовИерархияСтраницаПоСсылке: V82.СправочникиСсылка.РасходыБудущихПериодов,IReturn<РасходыБудущихПериодовИерархияСтраницаПоСсылке>
	{
		public Guid ___Родитель {get; set;}
		public int ___Режим {get; set;}
		public int ___Размер {get; set;}
	...
	public partial class РасходыБудущихПериодовСервис : Service
	{
		...
		public object Get(РасходыБудущихПериодовНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.РасходыБудущихПериодов.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new РасходыБудущихПериодовОтвет() { Ответ = "РасходыБудущихПериодов c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(РасходыБудущихПериодовИерархияВыбратьПоСсылке Запрос)
		{
			if (Запрос.___Макс == Guid.Empty)
			{
				Запрос.___Макс = Guid.Parse("ffffffff-ffff-ffff-ffff-ffffffffffff");
			...
			var Коллекция = new List<object>();
			foreach (var Ссылка in V82.Справочники.РасходыБудущихПериодов.ИерархияВыбратьПоСсылке(Запрос.___Родитель,Запрос.___Режим,Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
				Коллекция.Add(Ссылка.ПолучитьОбъект());
			}
			return Коллекция;
		...
		public object Get(РасходыБудущихПериодовИерархияСтраницаПоСсылке Запрос)
		{
			return null;
		}
