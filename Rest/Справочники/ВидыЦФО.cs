...
namespace V82.Справочники
{
	...
	[Маршрут("Справочники/ВидыЦФО/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}")]
	[Маршрут("Справочники/ВидыЦФО/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}/{___Мин}")]
	[Маршрут("Справочники/ВидыЦФО/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}/{___Мин}/{___Макс}")]
	public class ВидыЦФОИерархияВыбратьПоСсылке: V82.СправочникиСсылка.ВидыЦФО,IReturn<ВидыЦФОИерархияВыбратьПоСсылке>
	{
	...
	[Маршрут("Справочники/ВидыЦФО/ИерархияСтраницаПоСсылке","{___Родитель}/{___Режим}/{___Размер}/{___Номер}")]
	public class ВидыЦФОИерархияСтраницаПоСсылке: V82.СправочникиСсылка.ВидыЦФО,IReturn<ВидыЦФОИерархияСтраницаПоСсылке>
	{
		public Guid ___Родитель {get; set;}
		public int ___Режим {get; set;}
		public int ___Размер {get; set;}
	...
	public partial class ВидыЦФОСервис : Service
	{
		...
		public object Get(ВидыЦФОНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ВидыЦФО.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ВидыЦФООтвет() { Ответ = "ВидыЦФО c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ВидыЦФОВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.ВидыЦФО.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
		...
		public object Get(ВидыЦФОИерархияВыбратьПоСсылке Запрос)
		{
			if (Запрос.___Макс == Guid.Empty)
			{
				Запрос.___Макс = Guid.Parse("ffffffff-ffff-ffff-ffff-ffffffffffff");
			...
			var Коллекция = new List<object>();
			foreach (var Ссылка in V82.Справочники.ВидыЦФО.ИерархияВыбратьПоСсылке(Запрос.___Родитель,Запрос.___Режим,Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
				Коллекция.Add(Ссылка.ПолучитьОбъект());
			}
			return Коллекция;
		...
		public object Get(ВидыЦФОИерархияСтраницаПоСсылке Запрос)
		{
			return null;
		}
