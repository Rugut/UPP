...
namespace V82.Справочники
{
	...
	public partial class ОрганизацииСервис : Service
	{
		...
		public object Get(ОрганизацииНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.Организации.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ОрганизацииОтвет() { Ответ = "Организации c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ОрганизацииВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.Организации.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
