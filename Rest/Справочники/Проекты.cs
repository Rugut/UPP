...
namespace V82.Справочники
{
	...
	[Маршрут("Справочники/Проекты/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}")]
	[Маршрут("Справочники/Проекты/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}/{___Мин}")]
	[Маршрут("Справочники/Проекты/ИерархияВыбратьПоСсылке","{___Родитель}/{___Режим}/{___Первые}/{___Мин}/{___Макс}")]
	public class ПроектыИерархияВыбратьПоСсылке: V82.СправочникиСсылка.Проекты,IReturn<ПроектыИерархияВыбратьПоСсылке>
	{
	...
	[Маршрут("Справочники/Проекты/ИерархияСтраницаПоСсылке","{___Родитель}/{___Режим}/{___Размер}/{___Номер}")]
	public class ПроектыИерархияСтраницаПоСсылке: V82.СправочникиСсылка.Проекты,IReturn<ПроектыИерархияСтраницаПоСсылке>
	{
		public Guid ___Родитель {get; set;}
		public int ___Режим {get; set;}
		public int ___Размер {get; set;}
	...
	public partial class ПроектыСервис : Service
	{
		...
		public object Get(ПроектыНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.Проекты.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ПроектыОтвет() { Ответ = "Проекты c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ПроектыИерархияВыбратьПоСсылке Запрос)
		{
			if (Запрос.___Макс == Guid.Empty)
			{
				Запрос.___Макс = Guid.Parse("ffffffff-ffff-ffff-ffff-ffffffffffff");
			...
			var Коллекция = new List<object>();
			foreach (var Ссылка in V82.Справочники.Проекты.ИерархияВыбратьПоСсылке(Запрос.___Родитель,Запрос.___Режим,Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
				Коллекция.Add(Ссылка.ПолучитьОбъект());
			}
			return Коллекция;
		...
		public object Get(ПроектыИерархияСтраницаПоСсылке Запрос)
		{
			return null;
		}
