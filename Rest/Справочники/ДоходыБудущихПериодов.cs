...
namespace V82.Справочники
{
	...
	public partial class ДоходыБудущихПериодовСервис : Service
	{
		...
		public object Get(ДоходыБудущихПериодовНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.ДоходыБудущихПериодов.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new ДоходыБудущихПериодовОтвет() { Ответ = "ДоходыБудущихПериодов c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(ДоходыБудущихПериодовВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.ДоходыБудущихПериодов.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
