...
namespace V82.Справочники
{
	...
	public partial class СчетчикДляФормированияСерийныхНомеровСервис : Service
	{
		...
		public object Get(СчетчикДляФормированияСерийныхНомеровНайтиПоСсылке Запрос)
		{
			...
			var Ссылка = V82.Справочники.СчетчикДляФормированияСерийныхНомеров.НайтиПоСсылке(Запрос.Ссылка);
			if (Ссылка == null)
			{
				return new СчетчикДляФормированияСерийныхНомеровОтвет() { Ответ = "СчетчикДляФормированияСерийныхНомеров c ссылкой '" + Запрос.Ссылка + "' не найден." };
			}
		...
		public object Get(СчетчикДляФормированияСерийныхНомеровВыбратьПоСсылке Запрос)
		{
			...
			foreach (var Ссылка in V82.Справочники.СчетчикДляФормированияСерийныхНомеров.ВыбратьПоСсылке(Запрос.___Первые, Запрос.___Мин, Запрос.___Макс))
			{
