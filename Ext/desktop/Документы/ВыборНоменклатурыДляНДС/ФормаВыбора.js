...
{
	Ext.define('Документы.ВыборНоменклатурыДляНДС.ФормаВыбора',
	{
	...
	[{
		...
		[
		{
			...
			store:
			{
				...
				restful: true,
				autoSync: false,
				proxy: {type: 'jsonp',url: 'https://localhost:1337/Справочники/ВыборНоменклатурыДляНДС/ВыбратьПоСсылке/100', timeout: 3},
				fields:
				[
					{
						name:'Картинка',
			...
			listeners:
			{
				dblclick:
				{
					...
					fn: function ()
					{
						...
						Ext.require(['Справочники.ВыборНоменклатурыДляНДС.ФормаВыбораСобытия'], function ()
						{
							var obj = Ext.create("Справочники.ВыборНоменклатурыДляНДС.ФормаВыбораСобытия");
