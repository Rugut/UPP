	...
	[{
		...
		[
		{
			...
			store:
			{
				...
				restful: true,
				autoSync: false,
				proxy: {type: 'jsonp',url: 'https://localhost:1337/Справочники/ПричиныЗакрытияЗаказов/ВыбратьПоСсылке/100', timeout: 3},
				fields:
				[
					{
						name:'Картинка',
			...
			listeners:
			{
				dblclick:
				{
					...
					fn: function ()
					{
						...
						Ext.require(['Справочники.ПричиныЗакрытияЗаказов.ФормаВыбораСобытия'], function ()
						{
							var obj = Ext.create("Справочники.ПричиныЗакрытияЗаказов.ФормаВыбораСобытия");
