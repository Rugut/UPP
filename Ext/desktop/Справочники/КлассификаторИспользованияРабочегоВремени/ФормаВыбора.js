		...
		[
		{
			...
			store:
			{
				data: Ext.create("Ext.data.Store",
				{
					...
					fields: ['Ссылка','Картинка','Наименование','ПолноеНаименование','РабочееВремя',]
				}).data.items,
				autoLoad: true,
				pageSize: 50,
				restful: true,
				autoSync: false,
				proxy: {type: 'jsonp',url: 'https://localhost:1337/Справочники/КлассификаторИспользованияРабочегоВремени/ВыбратьПоСсылке/100', timeout: 200},
				fields:
				[
					{
						name:'Ссылка',
			...
			listeners:
			{
				dblclick:
				{
					...
					fn: function ()
					{
						...
						Ext.require(['Справочники.КлассификаторИспользованияРабочегоВремени.ФормаВыбораСобытия'], function ()
						{
							var obj = Ext.create("Справочники.КлассификаторИспользованияРабочегоВремени.ФормаВыбораСобытия");
