	...
	[{
		...
		[
		{
			...
			store:
			{
				...
				{
					data: Ext.create("Данные.Справочники.ВариантыОтветовОпросов").data,
					fields: ['Ссылка','Картинка','Код','Наименование','ОценкаОтвета','ТребуетРазвернутыйОтвет',]
				}).data.items,
				autoLoad: true,
				pageSize: 50,
				restful: true,
				autoSync: false,
				proxy: {type: 'jsonp',url: 'https://localhost:1337/Справочники/ВариантыОтветовОпросов/ВыбратьПоСсылке/100', timeout: 200},
				fields:
				[
					{
						name:'Ссылка',
			...
			listeners:
			{
				dblclick:
				{
					...
					fn: function ()
					{
						...
						Ext.require(['Справочники.ВариантыОтветовОпросов.ФормаСпискаСобытия'], function ()
						{
							var obj = Ext.create("Справочники.ВариантыОтветовОпросов.ФормаСпискаСобытия");
