		...
		[
		{
			...
			store:
			{
				data: Ext.create("Ext.data.Store",
				{
					...
					fields: ['Ссылка','Родитель.Представление','Картинка','Наименование','ДлительностьКурса','ЗатратыНаОдногоОбучающегося','Валюта.Представление',]
				}).data.items,
				autoLoad: true,
				pageSize: 50,
				restful: true,
			...
			listeners:
			{
				dblclick:
				{
					...
					fn: function ()
					{
						...
						Ext.require(['Справочники.КурсыОбучения.ФормаСпискаСобытия'], function ()
						{
							var obj = Ext.create("Справочники.КурсыОбучения.ФормаСпискаСобытия");
