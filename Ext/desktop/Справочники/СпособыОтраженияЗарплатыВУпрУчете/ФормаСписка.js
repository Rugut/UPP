		...
		[
		{
			...
			store:
			{
				data: Ext.create("Ext.data.Store",
				{
					...
					fields: ['Ссылка','Картинка','Наименование','СтатьяЗатрат.Представление','НоменклатурнаяГруппа.Представление','ОбъектСтроительства.Представление',]
				}).data.items,
				autoLoad: true,
				pageSize: 50,
				restful: true,
			...
			listeners:
			{
				dblclick:
				{
					...
					fn: function ()
					{
						...
						Ext.require(['Справочники.СпособыОтраженияЗарплатыВУпрУчете.ФормаСпискаСобытия'], function ()
						{
							var obj = Ext.create("Справочники.СпособыОтраженияЗарплатыВУпрУчете.ФормаСпискаСобытия");
