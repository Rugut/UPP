		...
		[
		{
			...
			[
				...
				},
				{
					text:'Вид события ОС',
					width:'100',
					dataIndex:'ВидСобытияОС.Представление',
					flex:1,
				},
			],
			store:
			{
				data: Ext.create("Ext.data.Store",
				{
					...
					fields: ['Ссылка','Картинка','Код','Наименование','ВидСобытияОС.Представление',]
				}).data.items,
				autoLoad: true,
				pageSize: 50,
				restful: true,
				autoSync: false,
				proxy: {type: 'jsonp',url: 'https://localhost:1337/Справочники/СобытияОС/ВыбратьПоСсылке/100', timeout: 200},
				fields:
				[
					{
						name:'Ссылка',
			...
			listeners:
			{
				dblclick:
				{
					...
					fn: function ()
					{
						...
						Ext.require(['Справочники.СобытияОС.ФормаСпискаСобытия'], function ()
						{
							var obj = Ext.create("Справочники.СобытияОС.ФормаСпискаСобытия");
