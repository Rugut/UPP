	...
	[{
		...
		[
		{
			...
			store:
			{
				...
				{
					data: Ext.create("Данные.Справочники.Кассы").data,
					fields: ['Ссылка','Родитель.Представление','Картинка','Код','Наименование','ВалютаДенежныхСредств.Представление','Владелец',]
				}).data.items,
				autoLoad: true,
				pageSize: 50,
				restful: true,
				autoSync: false,
				proxy: {type: 'jsonp',url: 'https://localhost:1337/Справочники/Кассы/ВыбратьПоСсылке/100', timeout: 200},
				fields:
				[
					{
						name:'Ссылка',
			...
			listeners:
			{
				dblclick:
				{
					...
					fn: function ()
					{
						...
						Ext.require(['Справочники.Кассы.ФормаСпискаСобытия'], function ()
						{
							var obj = Ext.create("Справочники.Кассы.ФормаСпискаСобытия");
		...
		},
		{
			...
			onTriggerClick : function(ЭтотОбъект)
			{
				...
				if (СтрокаЗнч.indexOf("-select-") != -1)
				{
					Ext.require(['Справочники.Кассы.ФормаСпискаСобытия'], function ()
					{
						var объект = Ext.create("Справочники.Кассы.ФормаСпискаСобытия");
