	...
	[{
					...
					[
		{
			...
			onTriggerClick : function(ЭтотОбъект)
			{
				...
				{
					Ext.require(['Отчеты.РегистрУчетаПрямыхРасходовНаПроизводство.ФормаНастройкаСобытия'], function ()
					{
						var объект = Ext.create("Отчеты.РегистрУчетаПрямыхРасходовНаПроизводство.ФормаНастройкаСобытия");
		...
		},
		{
			...
			store:
			{
				...
				restful: true,
				autoSync: false,
				proxy: {type: 'jsonp',url: 'https://localhost:1337/Справочники/РегистрУчетаПрямыхРасходовНаПроизводство/ВыбратьПоСсылке/100', timeout: 200},
				fields:
				[
					{
						name:'Ссылка',
		...
		},
		{
			xtype: 'label',
			text: 'Показатели',
			style: 'position:absolute;left:6px;top:85px;width:344px;height:16px;border-bottom: 2px solid maroon; color: #9F6500; border-width:thin ; border-color: #B3AC86; font-weight: 600;'
		},
		{
			xtype: 'label',
			name: 'Надпись1',
